...
const actor: BotActorContext = {
  guildId: '900000000000006600',
  discordUserId: '900000000000006601',
  interactionId: '900000000000006609',
  clientSource: 'DISCORD_BOT'
};
const result = {
  giftCatalogVersionId: '00000000-0000-0000-0000-000000006610',
  catalogVersion: 4,
  priceMinor: 8_800,
  recipientCount: 1,
  totalPriceMinor: 8_800,
  ledgerBalanceMinor: 5_000,
  reservedMinor: 1_200,
  availableMinor: 3_800,
  shortfallMinor: 5_000,
  currency: 'CAT',
  calculatedAt: '2026-07-19T21:00:00.000Z',
  stale: false,
  canAfford: false,
...
describe('M6-US-06 Sapphire recharge continuation', () => {
  test('keeps every enabled catalog option selectable regardless of affordability', () => {
    const panel = buildGiftPanel({
      orderId: 'order-1',
      orderPublicId: 'P-1',
      receiver: { userId: 'player-1', displayName: '阿岚' },
      ...
      items: [
        ...
        {
          id: result.giftCatalogVersionId,
          code: 'BOX',
          name: '礼盒',
          version: 4,
          priceMinor: 8_800,
  ...
  test('renders an ephemeral deficit-only panel with support, refresh, and back controls', () => {
    ...
    expect(JSON.stringify(message.fields)).toContain('500.0 CAT');
    expect(JSON.stringify(message.fields)).not.toMatch(/总余额|可用余额|全部预留/u);
    ...
    for (const button of buttons)
      if (button.data.custom_id) expect(button.data.custom_id.length).toBeLessThanOrEqual(100);
  });

  test('blocks stale confirmation and shows confirm only for the current affordable snapshot', () => {
    expect(
      JSON.stringify(buildGiftAffordabilityMessage({ ...result, stale: true, canAfford: false }, 'ctx_stale'))
    ).not.toContain('gift:confirm');
    const affordable = buildGiftAffordabilityMessage(
      { ...result, ledgerBalanceMinor: 10_000, availableMinor: 8_800, shortfallMinor: 0, canAfford: true },
      'ctx_ready'
    );
    expect(JSON.stringify(affordable)).toContain('bc:gift:confirm:ctx_ready');
    expect(JSON.stringify(affordable.fields)).toMatch(/880\.0 CAT[\s\S]*确认/u);
  });
  ...
  test('uses an expiring HMAC short token with no receiver input or server-side registry', () => {
    ...
    const context = {
      orderId: '00000000-0000-0000-0000-000000006601',
      orderVersion: 7,
      giftCatalogVersionId: result.giftCatalogVersionId,
      catalogVersion: 4,
      priceMinor: 8_800
    };
    ...
    expect(() =>
      readGiftContinuationToken(token, { ...actor, discordUserId: '900000000000006699' }, secret, now())
    ).toThrow(/context/i);
    expect(() => readGiftContinuationToken(token, actor, secret, new Date('2026-07-19T21:30:01.000Z'))).toThrow(
      /expired/i
    );
  });
  ...
  test('uses the shared affordability API and sends no receiver or balance calculation input', async () => {
    ...
    expect(
      await client.checkGiftAffordability('order-1', result.giftCatalogVersionId, ['participant-1'], actor)
    ).toEqual(result);
  ...
  test('renders a native gift select and wires refresh/reconfirm through the shared API handler', async () => {
    const secret = 'm6-us-06-test-signing-secret-at-least-32-bytes';
    const message = buildGiftCatalogMessage(
      {
        orderId: '00000000-0000-0000-0000-000000006601',
        orderPublicId: 'P-6601',
        receiver: { userId: 'player-derived', displayName: '阿岚' },
        balance: {
          ledgerBalanceMinor: 5_000,
          reservedMinor: 10_200,
          availableMinor: -5_200,
          currency: 'CAT',
          calculatedAt: result.calculatedAt
        },
        recipients: [
          { participantId: '00000000-0000-0000-0000-000000006606', playerId: 'player-derived', displayName: '阿岚' }
        ],
        items: [
          {
            id: result.giftCatalogVersionId,
      ...
      ['00000000-0000-0000-0000-000000006606']
    );
    ...
    const orderPanel = buildServiceLifecyclePanelMessage({
      orderId: '00000000-0000-0000-0000-000000006601',
      publicId: 'P-6601',
      status: 'IN_SERVICE',
      version: 7,
      ...
      availableActions: [
        { key: 'CUSTOMER_SEND_GIFT', role: 'CUSTOMER', enabled: true, risk: 'SECONDARY', reasonCode: null }
      ],
      readiness: {
        customer: 'READY',
        player: 'READY',
        ...
        startedAt: result.calculatedAt,
        staffTaskId: null
      }
    });
    const openId = (
      orderPanel.components[0]!.components.find((item) => item.type === 'BUTTON' && item.label === '赠送礼物') as any
    ).customId;
    expect(parseServiceCenterCustomId(openId)).toEqual({
      area: 'gift',
      action: 'open',
      orderId: '00000000-0000-0000-0000-000000006601',
      expectedVersion: 7
  ...
  test('paginates recipient selection without imposing an API recipient limit', () => {
    ...
    const data = {
      orderId: '00000000-0000-0000-0000-000000006601',
      orderPublicId: 'P-6601',
      receiver: { userId: recipients[0]!.playerId, displayName: '30 位订单陪玩' },
      recipients,
      balance: {
...
function now() {
  return new Date('2026-07-19T21:01:00.000Z');
}

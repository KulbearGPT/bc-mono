...
describe('M4-US-09 dashboard operating metrics', () => {
  ...
  test('AT-MET-001 returns exactly eight metrics and integer basis points', async () => {
    ...
    const result = await store.getSummary({ actorStaffId: 'staff-l3', actorLevel: 'L3_OPERATIONS', guildId: 'guild-1', now, timeZone: 'Asia/Shanghai', currency: 'USD' });
    expect(result).toMatchObject({ timeZone: 'Asia/Shanghai', currency: 'USD', metrics: {
  ...
  test('returns zero success for no valid dispatch round and hides money from L1', async () => {
    ...
    const result = await store.getSummary({ actorStaffId: 'staff-l1', actorLevel: 'L1_SUPPORT', guildId: 'guild-1', now, timeZone: 'Asia/Shanghai', currency: 'USD' });

...
function configApi(
  overrides: Partial<BotConfigApiClient> = {},
): BotConfigApiClient {
  return {
...
describe("M4-US-10 Discord /bot-config adapter", () => {
  test("uses the four OpenAPI operations with trusted actor headers and idempotency", async () => {
    const fetchMock = vi
      ...
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ data: { mayApply: true } }),
      })
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ data: { version: 13 } }),
      })
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ data: { delivered: true } }),
      });
    ...
    const change = {
      ...
      reason: "Discord Bot configuration update.",
    };

    await client.getBotConfig(guildId, actor);
    await client.validateBotConfigChange(
      change,
      actor,
      "discord:bot-config:validate:one",
    );
    await client.updateBotConfig(
      { ...change, validationToken: "token-value" },
      actor,
      "discord:bot-config:update:one",
    );
    await client.testBotConfigDelivery(
      {
        guildId,
        expectedVersion: 12,
        channelField: "gift_broadcast_channel_id",
        channelId: "555555555555555555",
        reason: "Discord Bot delivery test.",
      },
      actor,
      "discord:bot-config:test:one",
    );
    ...
    expect(fetchMock.mock.calls[1]?.[1]?.headers).toMatchObject({
      "idempotency-key": "discord:bot-config:validate:one",
    });
    expect(fetchMock.mock.calls[2]?.[1]?.headers).toMatchObject({
      "idempotency-key": "discord:bot-config:update:one",
    });
    expect(fetchMock.mock.calls[3]?.[1]?.headers).toMatchObject({
      "idempotency-key": "discord:bot-config:test:one",
    });
  });
  ...
  test("opens a private field picker and transitions to native Channel and Role Select components", async () => {
    const flow = new BotConfigFlow({
      ...
      sessions: new BotConfigSessionStore({ idFactory: () => "abc123XYZ0" }),
    });
    ...
    expect(opened.components[0]?.components[0]).toMatchObject({
      ...
      options: expect.arrayContaining([
        { label: "礼物播报频道", value: "gift_broadcast_channel_id" },
      ]),
    });
    expect(opened.components[1]?.components[0]).toMatchObject({
      ...
      options: expect.arrayContaining([
        { label: "基础玩家角色", value: "player_role_id" },
      ]),
    });

    const channel = flow.chooseField(
      actor,
      "abc123XYZ0",
  ...
  test("supports timeout, template and feature-toggle fields without exceeding Discord select limits", async () => {
    ...
    const api = configApi({
      getBotConfig: vi.fn().mockResolvedValue(scalarSnapshot),
    });
    const flow = new BotConfigFlow({
      api,
      cache: new BotConfigCache(),
    ...
    expect(
      flow.chooseField(actor, "abc123XYZ0", "new_orders_enabled").components[0]
        ?.components[0],
    ).toMatchObject({ type: "STRING_SELECT" });
    expect(
      flow.chooseField(actor, "abc123XYZ0", "dispatch_timeout_minutes")
        .components[0]?.components[0],
    ).toMatchObject({ type: "BUTTON", customId: "bc:cfg:input:abc123XYZ0" });
    const preview = await flow.previewTextInput(
      actor,
      "abc123XYZ0",
      "10",
      "discord:bot-config:validate:scalar",
    );
    expect(api.validateBotConfigChange).toHaveBeenCalledWith(
      expect.objectContaining({ changes: { dispatch_timeout_minutes: 10 } }),
      actor,
      "discord:bot-config:validate:scalar",
    );
  ...
  test("validates a Channel Select value, previews it, then confirms with the API token", async () => {
    ...
    const preview = await flow.previewValue(
      actor,
      "abc123XYZ0",
      "555555555555555555",
      "discord:bot-config:validate:777",
    );
    expect(api.validateBotConfigChange).toHaveBeenCalledWith(
      {
        guildId,
    ...
    const refreshed = {
      ...
      values: {
        ...current.values,
        gift_broadcast_channel_id: "555555555555555555",
      },
    };
    vi.mocked(api.getBotConfig).mockResolvedValueOnce(refreshed);
    const saved = await flow.confirm(
      actor,
      "abc123XYZ0",
      "discord:bot-config:update:777",
    );
    expect(api.updateBotConfig).toHaveBeenCalledWith(
  ...
  test("tests proposed channel delivery without applying the pending change", async () => {
    ...
    const result = await flow.testDelivery(
      actor,
      "abc123XYZ0",
      "discord:bot-config:test:777",
    );
  ...
  test("keeps full config, permissions and validation token out of short custom IDs and rejects another actor", async () => {
    ...
    expect(
      ids.every((id) => /^bc:cfg:[a-z-]+:[A-Za-z0-9_-]{8,16}$/u.test(id)),
    ).toBe(true);
    expect(ids.join("|")).not.toMatch(
      /gift_broadcast|player_role|permission|validation-token|333333333333333333/iu,
    );
    expect(parseBotConfigCustomId("bc:cfg:confirm:abc123XYZ0")).toEqual({
      operation: "confirm",
      sessionId: "abc123XYZ0",
    });
    const stored = (
      sessions as unknown as { sessions: Map<string, Record<string, unknown>> }
    ).sessions.get("abc123XYZ0");
    expect(stored).toEqual(
      expect.objectContaining({ guildId, discordUserId, version: 12 }),
    );
  ...
  test("cold reload fetches current Guild config from API instead of relying on old memory", async () => {
    ...
    expect(api.getBotConfig).toHaveBeenCalledWith(guildId, {
      guildId,
  ...
  test("exports the module and discovers the Guild command, component handlers, and ready reload path", async () => {
    ...
    const packageJson = JSON.parse(
      await readFile("apps/bot/package.json", "utf8"),
    );
    ...
    expect(manifest.pieces).toEqual(
      expect.arrayContaining([
        ...
        expect.objectContaining({
          ...
          name: "bot-config-selects",
        }),
        expect.objectContaining({
          kind: "interaction-handlers",
          name: "bot-config-buttons",
        }),
        expect.objectContaining({
          kind: "interaction-handlers",
          name: "bot-config-modals",
        }),
      ]),
    );
    ...
    expect(selectSource).toMatch(
      /isChannelSelectMenu\(\)|isRoleSelectMenu\(\)/u,
    );
    expect(`${commandSource}\n${selectSource}\n${buttonSource}`).not.toMatch(
      /setDefaultMemberPermissions|L3_OPERATIONS|L4_ADMIN_OWNER/u,
    );
    expect(`${commandSource}\n${selectSource}\n${buttonSource}`).not.toMatch(
      /ephemeral:\s*false/u,
    );
    expect(commandSource.indexOf("deferReply")).toBeLessThan(
      commandSource.indexOf("botConfigFlow.open"),
    );
    expect(selectSource.indexOf("deferUpdate")).toBeLessThan(
      selectSource.indexOf("previewValue"),
    );

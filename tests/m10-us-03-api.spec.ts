...
import { buildAddOrderParticipantRequest, buildUpdateAdminOrderNoteRequest, buildUpdateAdminOrderRequirementRequest, buildUpdateOrderParticipantRequest } from '../apps/dashboard/src/admin-business.js';
...
describe('M10-US-03 order participant API', () => {
  test('builds Dashboard participant writes without accepting a client-derived order total', () => {
    ...
    expect(buildUpdateAdminOrderNoteRequest(orderId,{expectedOrderVersion:3,note:'老板稍后上线',reasonCode:'SUPPORT_CORRECTION'})).toEqual({method:'PATCH',path:`/api/v1/admin/orders/${orderId}`,body:{expectedOrderVersion:3,action:'CHANGE_NOTE',note:'老板稍后上线',reasonCode:'SUPPORT_CORRECTION'}});
    expect(buildUpdateAdminOrderRequirementRequest(orderId,'requirement-a',{expectedOrderVersion:4,expectedRequirementVersion:2,customerNote:'改为中文沟通',reasonCode:'SUPPORT_CORRECTION'})).toEqual({method:'PATCH',path:`/api/v1/admin/orders/${orderId}/requirements/requirement-a`,body:{expectedOrderVersion:4,expectedRequirementVersion:2,action:'CHANGE_NOTE',customerNote:'改为中文沟通',reasonCode:'SUPPORT_CORRECTION'}});
  });
  ...
  test('renders every participant project, price and compensation inside the order overlay',()=>{
    ...
    const html=renderToStaticMarkup(createElement(AdminBusinessPage,{model,detail:{kind:'READY',page:'orders',requestId:'req-m10',data:{order:{id:orderId,publicId:'P-M10',version:3,status:'DRAFT',notes:'老板稍后上线',customerId:'customer-1',customerDiscordUserId:'123456789012345678',customerDiscordTag:'老板猫#1024',sourcePackageVersionId:'package-v2',sourcePackageCode:'DELTA_ESCORT',sourcePackageDisplayName:'三角洲护航',sourcePackageVersion:2,compositionMode:'CUSTOMIZED',amountMinor:540,currency:'CAT'},timeline:{items:[],nextCursor:null},requirements:{catalogSubtotalMinor:560,packageAdjustmentMinor:-20,derivedTotalMinor:540,items:[{id:'requirement-a',version:1,sourcePackageSlotId:'slot-a',serviceCatalogVersionId:valorantTech,customerNote:'负责技术护航',gameDisplayName:'瓦洛兰特',serviceDisplayName:'技术陪玩',regionDisplayName:'北美',billingUnitMinutes:60,unitCount:2,requestedPlayerCount:1,filledPlayerCount:1,estimatedLinePriceMinor:240,status:'ACTIVE'},{id:'requirement-b',version:1,sourcePackageSlotId:'slot-b',serviceCatalogVersionId:lolFun,customerNote:'技术要求不高，会聊天就行',gameDisplayName:'英雄联盟美服',serviceDisplayName:'娱乐陪玩',billingUnitMinutes:60,unitCount:3,requestedPlayerCount:2,filledPlayerCount:1,estimatedLinePriceMinor:300,status:'ACTIVE'}]},participants:{derivedTotalMinor:540,items:[{id:'participant-a',orderRequirementId:'requirement-a',playerId:playerA,discordUserId:'223456789012345678',discordTag:'奶糖#2048',displayName:'奶糖',serviceCatalogVersionId:valorantTech,gameDisplayName:'瓦洛兰特',serviceDisplayName:'技术陪玩',regionDisplayName:'北美',billingUnitMinutes:60,unitCount:2,linePriceMinor:240,compensationType:'PERCENT_BPS',compensationValue:6000,compensationSource:'CATALOG_DEFAULT',expectedEarningMinor:144,status:'ACTIVE',readiness:'NOT_READY',version:1},{id:'participant-b',orderRequirementId:'requirement-b',playerId:playerB,discordUserId:'323456789012345678',discordTag:'布丁#4096',displayName:'布丁',serviceCatalogVersionId:lolFun,gameDisplayName:'英雄联盟美服',serviceDisplayName:'娱乐陪玩',billingUnitMinutes:60,unitCount:3,linePriceMinor:300,compensationType:'FIXED_MINOR',compensationValue:55,compensationSource:'PLAYER_OVERRIDE',expectedEarningMinor:165,status:'ACTIVE',readiness:'READY',version:1}]}}},serviceCatalogOptions:[],participantPlayerOptions:[],onAddOrderParticipant:()=>undefined,onUpdateOrderParticipant:()=>undefined,onUpdateOrderNote:()=>undefined,onUpdateOrderRequirement:()=>undefined}));
    expect(html).toContain('项目需求');expect(html).toContain('还差 1 位');expect(html).toContain('老板猫#1024');expect(html).toContain('三角洲护航');expect(html).toContain('DELTA_ESCORT');expect(html).toContain('技术要求不高，会聊天就行');expect(html).toContain('套餐调整 -2.0 猫条');expect(html).toContain('奶糖#2048');expect(html).toContain('陪玩与项目');expect(html).toContain('瓦洛兰特 · 技术陪玩');expect(html).toContain('英雄联盟美服 · 娱乐陪玩');expect(html).toContain('60.00% · 项目默认');expect(html).toContain('5.5 猫条/单位 · 个人规则');expect(html).toContain('编辑订单备注');expect(html).toContain('编辑席位备注');expect(html).toContain('编辑明细');expect(html).toContain('改派陪玩');
  });

  test('AT-MULTI-015 lets scoped staff correct or clear notes while terminal orders stay immutable',async()=>{
    const requirementId='00000000-0000-0000-0000-000000004001';
    const store=new InMemoryOrderRequirementStore({orders:[{id:orderId,guildId,customerDiscordUserId:'123456789012345678',status:'PENDING_DISPATCH',version:1,amountMinor:200,notes:'旧备注'}],claimedOrderIdsByStaffId:{'00000000-0000-0000-0000-111111111111':[orderId]},catalogs:[{id:valorantTech,status:'ACTIVE',game:'VALORANT',gameDisplayName:'瓦洛兰特',service:'TECH',serviceDisplayName:'技术陪玩',region:'NA',regionDisplayName:'北美',billingUnitMinutes:60,customerUnitPriceMinor:100}],requirements:[{id:requirementId,orderId,sourcePackageSlotId:null,serviceCatalogVersionId:valorantTech,game:'VALORANT',gameDisplayName:'瓦洛兰特',service:'TECH',serviceDisplayName:'技术陪玩',region:'NA',regionDisplayName:'北美',billingUnitMinutes:60,unitCount:2,requestedPlayerCount:1,customerUnitPriceMinor:100,estimatedLinePriceMinor:200,filledPlayerCount:0,customerNote:'旧席位备注',status:'ACTIVE',version:1,createdAt:'2026-08-10T00:00:00.000Z',updatedAt:'2026-08-10T00:00:00.000Z'}]});
    const server=buildApiServer({env,security:{staffDirectory},orderRequirements:{store}});
    const unclaimed=await server.inject({method:'PATCH',url:`/api/v1/admin/orders/${orderId}`,headers:headers(unclaimedL1DiscordId,'m10:order-note:unclaimed'),payload:{expectedOrderVersion:1,action:'CHANGE_NOTE',note:'不得越权写入',reasonCode:'SUPPORT_CORRECTION'}});
    expect(unclaimed.statusCode).toBe(403);expect(store.orders[0]!.notes).toBe('旧备注');
    const orderNote=await server.inject({method:'PATCH',url:`/api/v1/admin/orders/${orderId}`,headers:headers(l1DiscordId,'m10:order-note:1'),payload:{expectedOrderVersion:1,action:'CHANGE_NOTE',note:'老板稍后上线',reasonCode:'SUPPORT_CORRECTION'}});
    expect(orderNote.statusCode,orderNote.body).toBe(200);expect(orderNote.json().data).toMatchObject({orderVersion:2,note:'老板稍后上线'});
    const requirementNote=await server.inject({method:'PATCH',url:`/api/v1/admin/orders/${orderId}/requirements/${requirementId}`,headers:headers(l2DiscordId,'m10:requirement-note:1'),payload:{expectedOrderVersion:2,expectedRequirementVersion:1,action:'CHANGE_NOTE',customerNote:null,reasonCode:'SUPPORT_CORRECTION'}});
    expect(requirementNote.statusCode,requirementNote.body).toBe(200);expect(requirementNote.json().data).toMatchObject({orderVersion:3,requirement:{version:2,customerNote:null}});expect(store.orders[0]).toMatchObject({version:3,amountMinor:200,notes:'老板稍后上线'});
    store.orders[0]!.status='CANCELLED';
    const terminal=await server.inject({method:'PATCH',url:`/api/v1/admin/orders/${orderId}`,headers:headers(l2DiscordId,'m10:order-note:terminal'),payload:{expectedOrderVersion:3,action:'CHANGE_NOTE',note:'不应写入',reasonCode:'SUPPORT_CORRECTION'}});
    expect(terminal.statusCode).toBe(422);expect(store.orders[0]!.notes).toBe('老板稍后上线');
    store.orders[0]!.status='PENDING_DISPATCH';store.orders[0]!.captured=true;
    const captured=await server.inject({method:'PATCH',url:`/api/v1/admin/orders/${orderId}`,headers:headers(l2DiscordId,'m10:order-note:captured'),payload:{expectedOrderVersion:3,action:'CHANGE_NOTE',note:'不得修改已捕获订单',reasonCode:'SUPPORT_CORRECTION'}});
    expect(captured.statusCode).toBe(422);expect(store.orders[0]!.notes).toBe('老板稍后上线');
    await server.close();
  });
  ...
  test('enforces claimed-task and guild scope and rejects client totals, duplicate players, ineligible players and captured orders', async () => {
    ...
    store.orders[0]!.captured=false;store.orders[0]!.status='CANCELLED';
    const terminal=await server.inject({method:'POST',url:`/api/v1/admin/orders/${orderId}/participants`,headers:headers(l2DiscordId,'m10:participant:test:terminal'),payload:{playerId:playerA,serviceCatalogVersionId:valorantTech,unitCount:2,linePriceMinor:240,expectedOrderVersion:1,reasonCode:'ADD_PLAYER'}});
    expect(terminal.statusCode).toBe(422);expect(store.participants).toHaveLength(0);
  });
});

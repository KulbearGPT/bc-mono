...
describe('M10-US-03 order participant API', () => {
  ...
  test('renders every participant project, price and compensation inside the order overlay',()=>{
    ...
    expect(html).toContain('项目需求');expect(html).toContain('还差 1 位');expect(html).toContain('老板猫#1024');expect(html).toContain('三角洲护航');expect(html).toContain('DELTA_ESCORT');expect(html).toContain('技术要求不高，会聊天就行');expect(html).toContain('套餐调整 -2.0 猫条');expect(html).toContain('奶糖#2048');expect(html).toContain('陪玩与项目');expect(html).toContain('瓦洛兰特 · 技术陪玩');expect(html).toContain('英雄联盟美服 · 娱乐陪玩');expect(html).toContain('60.00% · 项目默认');expect(html).toContain('5.5 猫条/单位 · 个人规则');expect(html).toContain('编辑明细');
  });

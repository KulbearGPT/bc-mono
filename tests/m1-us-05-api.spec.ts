...
describe('M1-US-05 internal wallet reservation contract',()=>{
  test('submits against one USD balance fact without capturing funds',async()=>{
    ...
    await wallet.createTopUp({userId,amountMinor:200_000,paymentChannel:'stripe',externalTransactionId:'pi_m1_05',paidAt:now.toISOString(),note:'verified',
      idempotencyKey:'m1:05:topup',actorStaffId:'00000000-0000-0000-0000-000000001506',actorLevel:'L1_SUPPORT',now});
    ...
    expect(reserved.balance).toMatchObject({ledgerBalanceMinor:200_000,reservedMinor:120_000,availableMinor:80_000,currency:'USD'});
  ...
  test('rejects a competing reservation that exceeds available balance',async()=>{
    ...
    await wallet.createTopUp({userId,amountMinor:100,paymentChannel:'cash',externalTransactionId:'cash_m1_05',paidAt:now.toISOString(),note:'verified',
      idempotencyKey:'m1:05:topup:2',actorStaffId:'00000000-0000-0000-0000-000000001516',actorLevel:'L1_SUPPORT',now});

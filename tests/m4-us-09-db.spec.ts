...
async function seed(){await pool.query(`
  ...
  INSERT INTO orders(id,public_id,customer_id,player_id,active_customer_slot_id,status,row_version,currency,amount_minor,guild_id,completed_at,created_at,updated_at) VALUES
    ('${ids.order}','P-0905','${ids.customer}','${ids.player}',NULL,'COMPLETED',3,'CAT',10000,'900000000000009000','2026-07-18T17:30:00Z','2026-07-18T16:30:00Z','2026-07-18T17:30:00Z'),
    ('${ids.active}','P-0906','${ids.activeCustomer}',NULL,'${ids.activeCustomer}','PENDING_DISPATCH',2,'CAT',12000,'900000000000009000',NULL,'2026-07-18T17:00:00Z','2026-07-18T17:00:00Z'),
    ('${ids.exception}','P-0907','${ids.exceptionCustomer}',NULL,'${ids.exceptionCustomer}','EXCEPTION',4,'CAT',5000,'900000000000009000',NULL,'2026-07-18T17:10:00Z','2026-07-18T17:20:00Z'),
    ('${ids.other}','P-0908','${ids.otherCustomer}',NULL,'${ids.otherCustomer}','PENDING_DISPATCH',2,'CAT',9000,'900000000000009999',NULL,'2026-07-18T17:00:00Z','2026-07-18T17:00:00Z');
  ...
  INSERT INTO consumption_entries(id,user_id,entry_type,direction,order_id,amount_minor,currency,source_type,source_id,idempotency_key,occurred_at) VALUES
    (gen_random_uuid(),'${ids.customer}','ORDER_CHARGE','DEBIT','${ids.order}',10000,'CAT','ORDER','${ids.order}','metric:order:charge','2026-07-18T17:30:00Z'),
    (gen_random_uuid(),'${ids.customer}','REFUND_REVERSAL','CREDIT','${ids.order}',2000,'CAT','REFUND',gen_random_uuid(),'metric:order:refund','2026-07-18T17:40:00Z');
  INSERT INTO fund_reservations(id,user_id,source_type,order_id,mode,provider,amount_minor,currency,status,row_version,idempotency_key,created_at,updated_at) VALUES ('${ids.reservation}','${ids.activeCustomer}','ORDER','${ids.active}','LOCAL_RESERVATION','mock',10000,'CAT','PARTIALLY_SETTLED',2,'metric:reservation',now(),now());
  ...
  INSERT INTO gift_catalog_versions(id,gift_catalog_item_id,version,status,active_gift_key,name,price_minor,currency,broadcast_template,created_by_staff_id,activated_at,created_at) VALUES ('${ids.giftVersion}','${ids.giftItem}',1,'ACTIVE','${ids.giftItem}','Metric Gift',3000,'CAT','{sender} sent {gift}','${ids.staff}',now(),now());
  INSERT INTO gift_requests(id,public_id,order_id,gift_catalog_version_id,sender_id,receiver_id,status,row_version,gift_code_snapshot,gift_name_snapshot,price_minor,currency,broadcast_template_snapshot,captured_at,expires_at,created_at,updated_at) VALUES ('${ids.gift}','G-0912','${ids.order}','${ids.giftVersion}','${ids.customer}','${ids.player}','CAPTURED',3,'METRIC_GIFT','Metric Gift',3000,'CAT','{sender} sent {gift}','2026-07-18T17:20:00Z','2026-07-18T18:20:00Z','2026-07-18T17:15:00Z','2026-07-18T17:20:00Z');
  INSERT INTO consumption_entries(id,user_id,entry_type,direction,gift_request_id,amount_minor,currency,source_type,source_id,idempotency_key,occurred_at) VALUES
    ('${ids.giftDebit}','${ids.customer}','GIFT_CHARGE','DEBIT','${ids.gift}',3000,'CAT','GIFT','${ids.gift}','metric:gift:charge','2026-07-18T17:20:00Z'),
    ('${ids.giftCredit}','${ids.customer}','REFUND_REVERSAL','CREDIT','${ids.gift}',500,'CAT','REFUND',gen_random_uuid(),'metric:gift:refund','2026-07-18T17:40:00Z');

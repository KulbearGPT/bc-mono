...
const recoveryOrderId='00000000-0000-0000-0000-000000010040';
const recoveryCustomerId='00000000-0000-0000-0000-000000010041';
const recoveryRequirementId='00000000-0000-0000-0000-000000010042';
...
describe('M10-US-03 PostgreSQL participant transaction', () => {
  ...
  test('AT-MULTI-015 persists staff note corrections without changing funds and rejects terminal recovery writes',async()=>{
    const directory:StaffDirectory={resolveByDiscord:()=>({staffId,userId:staffUserId,level:'L2_SUPERVISOR',permissionsVersion:1,status:'ACTIVE'})};const server=buildApiServer({env,security:{staffDirectory:directory,auditSink:new PostgresAuditSink({client:pool})},orderRequirements:{store:new PostgresOrderRequirementStore(pool),now:()=>new Date('2026-08-10T18:00:00Z')}});const common={authorization:'Bearer token','x-client-source':'DASHBOARD','x-actor-discord-user-id':discordId,'x-actor-guild-id':guildId};
    const orderNote=await server.inject({method:'PATCH',url:`/api/v1/admin/orders/${recoveryOrderId}`,headers:{...common,'idempotency-key':'m10:recovery:order-note'},payload:{expectedOrderVersion:1,action:'CHANGE_NOTE',note:'老板延后十分钟上线',reasonCode:'SUPPORT_CORRECTION'}});expect(orderNote.statusCode,orderNote.body).toBe(200);
    const requirementNote=await server.inject({method:'PATCH',url:`/api/v1/admin/orders/${recoveryOrderId}/requirements/${recoveryRequirementId}`,headers:{...common,'idempotency-key':'m10:recovery:requirement-note'},payload:{expectedOrderVersion:2,expectedRequirementVersion:1,action:'CHANGE_NOTE',customerNote:null,reasonCode:'SUPPORT_CORRECTION'}});expect(requirementNote.statusCode,requirementNote.body).toBe(200);
    const facts=await pool.query(`SELECT orders.customer_note,orders.amount_minor::text,orders.row_version,requirement.customer_note requirement_note,requirement.row_version requirement_version,(SELECT count(*)::int FROM order_events WHERE order_id=orders.id AND event_type='DETAILS_UPDATED') detail_events,(SELECT count(*)::int FROM order_requirement_events WHERE order_requirement_id=requirement.id AND event_type='NOTE_CHANGED') note_events,(SELECT count(*)::int FROM outbox_events WHERE order_id=orders.id AND event_type='PANEL_SYNC') panel_syncs,(SELECT count(*)::int FROM audit_logs WHERE target_id IN (orders.id::text,requirement.id::text) AND outcome='SUCCEEDED') audits FROM orders JOIN order_requirements requirement ON requirement.id=$2 WHERE orders.id=$1`,[recoveryOrderId,recoveryRequirementId]);expect(facts.rows[0]).toEqual({customer_note:'老板延后十分钟上线',amount_minor:'200',row_version:3,requirement_note:null,requirement_version:2,detail_events:2,note_events:1,panel_syncs:2,audits:2});
    await pool.query(`UPDATE orders SET status='CANCELLED',active_customer_slot_id=NULL,cancelled_at=now() WHERE id=$1`,[recoveryOrderId]);const terminal=await server.inject({method:'PATCH',url:`/api/v1/admin/orders/${recoveryOrderId}`,headers:{...common,'idempotency-key':'m10:recovery:terminal'},payload:{expectedOrderVersion:3,action:'CHANGE_NOTE',note:'不得保存',reasonCode:'SUPPORT_CORRECTION'}});expect(terminal.statusCode).toBe(422);const unchanged=await pool.query(`SELECT customer_note,row_version,(SELECT count(*)::int FROM outbox_events WHERE order_id=$1) outbox_count FROM orders WHERE id=$1`,[recoveryOrderId]);expect(unchanged.rows[0]).toEqual({customer_note:'老板延后十分钟上线',row_version:3,outbox_count:2});await server.close();
  });
  ...
  test('submits requirement pricing and atomically fills only the selected dispatch slot',async()=>{
    ...
    const headers={authorization:'Bearer token','x-client-source':'DISCORD_BOT','x-actor-discord-user-id':requirementDiscordId,'x-actor-guild-id':guildId,'x-discord-interaction-id':'555555555555555555'};
    const metadata=await server.inject({method:'PATCH',url:`/api/v1/orders/${requirementOrderId}`,headers:{...headers,'idempotency-key':'m10:requirement:metadata:0002'},payload:{expectedVersion:2,notes:'技术局优先沟通',preferredPlayerDiscordUserIds:['444444444444444444']}});expect(metadata.statusCode,metadata.body).toBe(200);expect(metadata.json().data).toMatchObject({version:3,amountMinor:600,notes:'技术局优先沟通',preferredPlayerDiscordUserIds:['444444444444444444']});
    const submitted=await server.inject({method:'POST',url:`/api/v1/orders/${requirementOrderId}/submit`,headers:{...headers,'idempotency-key':'m10:requirement:submit:0003'},payload:{expectedVersion:3}});expect(submitted.statusCode,submitted.body).toBe(200);expect(submitted.json().data).toMatchObject({status:'PENDING_DISPATCH',version:4,reservation:{amountMinor:600}});
    const dispatchStore=new PostgresDispatchStore({pool});const playerPool=new PostgresDispatchPlayerPool({pool});const dispatched=await dispatchOrder({orderStore,dispatchStore,playerPool,orderId:requirementOrderId,expectedVersion:4,trigger:'ORDER_SUBMITTED',dispatchChannelId:'777777777777777777',idempotencyKey:'m10:slot:dispatch:0004',now:new Date('2026-08-04T12:04:00Z')});
    ...
    const accepted=await acceptOrder({orderStore,dispatchStore,playerPool,orderId:requirementOrderId,expectedVersion:4,dispatchAttemptId:dispatched.dispatchAttemptId,orderRequirementId:requirement.id,actor:{guildId,discordUserId:'444444444444444444'},idempotencyKey:'m10:slot:accept:0005',now:new Date('2026-08-04T12:04:10Z')});expect(accepted.status).toBe('PENDING_DISPATCH');
    const facts=await pool.query(`SELECT orders.status::text,orders.row_version,participant.order_requirement_id,participant.player_id,attempt.order_requirement_id attempt_requirement_id FROM orders JOIN order_participants participant ON participant.order_id=orders.id JOIN dispatch_attempts attempt ON attempt.id=$2 WHERE orders.id=$1`,[requirementOrderId,dispatched.dispatchAttemptId]);expect(facts.rows[0]).toMatchObject({status:'PENDING_DISPATCH',row_version:5,order_requirement_id:requirement.id,player_id:playerId,attempt_requirement_id:requirement.id});

    const secondDispatch=await dispatchOrder({orderStore,dispatchStore,playerPool,orderId:requirementOrderId,expectedVersion:5,trigger:'ORDER_SUBMITTED',dispatchChannelId:'777777777777777777',idempotencyKey:'m10:slot:dispatch:0006',now:new Date('2026-08-04T12:05:00Z')});
    const secondCandidates=await pool.query(`SELECT player_user_id FROM dispatch_candidates WHERE dispatch_attempt_id=$1 ORDER BY player_user_id`,[secondDispatch.dispatchAttemptId]);expect(secondCandidates.rows).toEqual([{player_user_id:secondPlayerId},{player_user_id:thirdPlayerId}]);
    const race=await Promise.allSettled([
      acceptOrder({orderStore,dispatchStore,playerPool,orderId:requirementOrderId,expectedVersion:5,dispatchAttemptId:secondDispatch.dispatchAttemptId,actor:{guildId,discordUserId:'555555555555555555'},idempotencyKey:'m10:slot:race:a',now:new Date('2026-08-04T12:05:10Z')}),
      acceptOrder({orderStore,dispatchStore,playerPool,orderId:requirementOrderId,expectedVersion:5,dispatchAttemptId:secondDispatch.dispatchAttemptId,actor:{guildId,discordUserId:'666666666666666666'},idempotencyKey:'m10:slot:race:b',now:new Date('2026-08-04T12:05:10Z')})
    ]);
    expect(race.filter((result)=>result.status==='fulfilled'),race.map((result)=>result.status==='rejected'?String(result.reason):result.value.status).join(' | ')).toHaveLength(1);
    ...
    const thirdDispatch=await dispatchOrder({orderStore,dispatchStore,playerPool,orderId:requirementOrderId,expectedVersion:6,trigger:'ORDER_SUBMITTED',dispatchChannelId:'777777777777777777',idempotencyKey:'m10:slot:dispatch:0007',now:new Date('2026-08-04T12:06:00Z')});
    const remainingDiscordId=race[0]?.status==='fulfilled'?'666666666666666666':'555555555555555555';
    const finalAccepted=await acceptOrder({orderStore,dispatchStore,playerPool,orderId:requirementOrderId,expectedVersion:6,dispatchAttemptId:thirdDispatch.dispatchAttemptId,actor:{guildId,discordUserId:remainingDiscordId},idempotencyKey:'m10:slot:accept:0008',now:new Date('2026-08-04T12:06:10Z')});
...
async function seed(){await pool.query(`
  ...
  INSERT INTO orders(id,public_id,customer_id,active_customer_slot_id,status,row_version,amount_minor,expected_player_earning_minor,currency,guild_id,channel_id,panel_message_id,created_at,updated_at) VALUES
    ...
    ('${recoveryOrderId}','P-M10-RECOVERY','${recoveryCustomerId}','${recoveryCustomerId}','PENDING_DISPATCH',1,200,120,'CAT','${guildId}','111111111111111116','222222222222222228',now(),now());
  INSERT INTO order_requirements(id,order_id,service_catalog_version_id,status,row_version,game_code_snapshot,game_display_name_snapshot,service_code_snapshot,service_display_name_snapshot,region_code_snapshot,region_display_name_snapshot,billing_unit_minutes_snapshot,unit_count,requested_player_count,customer_unit_price_minor_snapshot,estimated_line_price_minor,customer_note,created_at,updated_at) VALUES('${recoveryRequirementId}','${recoveryOrderId}','${catalogId}','ACTIVE',1,'VALORANT','瓦洛兰特','TECH','技术陪玩','NA','北美',60,2,1,100,200,'旧席位备注',now(),now());

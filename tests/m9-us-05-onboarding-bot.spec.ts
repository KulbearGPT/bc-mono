...
describe('M9-US-05 persistent Discord newcomer entry adapter',()=>{
  test('renders exactly the two approved stable buttons',()=>{const message=buildOnboardingMessage();const json=message.components?.[0]?.toJSON() as {components:Array<{custom_id:string;label:string}>};
    expect(json.components.map(item=>[item.custom_id,item.label])).toEqual([[REGISTER_PLAYER_CUSTOM_ID,'注册为玩家'],[APPLY_COMPANION_CUSTOM_ID,'申请成为陪玩']]);});

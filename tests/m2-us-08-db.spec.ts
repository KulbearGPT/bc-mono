  ...
  test('scopes current orders, dispatch offers and earnings to the current player user id', async () => {
    const query = vi.fn()
      ...
      .mockResolvedValueOnce({ rows: [{
        ...
        player_earning_minor: 8000, currency: 'CAT', requirement_snapshot: { language: '中文' }, voice_channel_id: null
      }] })
      .mockResolvedValueOnce({ rows: [{ pending_minor: '8000', confirmed_minor: '3000', paid_minor: '20000', currency: 'CAT' }] });

...
async function fixture() {
  ...
  const walletFunding = new TestWalletFunding(8_000);
  walletFunding.addReservation('profile-order-reservation', 5_000);
  const server = buildApiServer({
    env: { NODE_ENV: 'test', DATABASE_URL: '', API_PORT: '0', API_BASE_URL: 'http://localhost:3000', BOT_SERVICE_TOKEN: 'bot-token' },
    security: {},
    account: { store: accountStore, walletFunding, profileStore, now: () => now }
  });
...
describe('M6-US-05 current-user profile API', () => {
  ...
  test('returns the current internal USD ledger balance without external snapshot state', async () => {
  ...
  test('uses a strict self-service whitelist and a non-enumerating 404 for an unbound actor', async () => {
    ...
    expect(response.json().data).not.toHaveProperty('rechargeUrl');

...
const accounts: Record<string, StaffAccount> = {
  l1: {
    staffId: "00000000-0000-0000-0000-000000010001",
    userId: "00000000-0000-0000-0000-000000010011",
    level: "L1_SUPPORT",
    permissionsVersion: 1,
    status: "ACTIVE",
  },
  l3: {
    staffId: "00000000-0000-0000-0000-000000010003",
    userId: "00000000-0000-0000-0000-000000010013",
    level: "L3_OPERATIONS",
    permissionsVersion: 1,
    status: "ACTIVE",
  },
  l4: {
    staffId: "00000000-0000-0000-0000-000000010004",
    userId: "00000000-0000-0000-0000-000000010014",
...
class FakeDiscordAdapter implements BotConfigDiscordAdapter {
  ...
  async validateObject(input: {
    ...
    return this.invalidIds.has(input.value)
      ? {
          ok: false as const,
          code: "MISSING_BOT_PERMISSION",
          message: "Bot cannot send to this channel.",
        }
      : { ok: true as const };
  }
...
function fixture() {
  ...
  let clock = now;
  const directory = {
    resolveByDiscord: ({ discordUserId }: { discordUserId: string }) =>
      accounts[discordUserId] ?? null,
  };
...
function headers(actor: keyof typeof accounts, write = false) {
  return {
    ...
    ...(write
      ? { "idempotency-key": `m4-us-10-${actor}-${crypto.randomUUID()}` }
      : {}),
  };
}

async function validate(
  server: ReturnType<typeof buildApiServer>,
  actor: "l3" | "l4",
...
describe("M4-US-10 Bot configuration API", () => {
  ...
  test("AT-CFG-001/002 exposes manageable fields to L3 and cumulative L4", async () => {
    ...
    const [l1, l3, l4] = await Promise.all([
      server.inject({
        method: "GET",
        url: `/api/v1/admin/bot-config?guildId=${guildId}`,
        headers: headers("l1"),
      }),
      server.inject({
        method: "GET",
    ...
    expect(l3.json().data.manageableFields).toContain(
      "gift_broadcast_channel_id",
    );
    expect(l3.json().data.manageableFields).not.toContain("staff_l4_role_id");
    expect(l4.json().data.manageableFields).toEqual(
      expect.arrayContaining(["gift_broadcast_channel_id", "staff_l4_role_id"]),
    );
  });
  ...
  test("AT-CFG-009 allows trusted Bot startup to reload a Guild without impersonating staff", async () => {
    ...
    expect(response.json().data).toMatchObject({
      guildId,
      version: 1,
      values: { gift_broadcast_channel_id: channelId },
    });
  });
  ...
  test("returns a controlled validation error for malformed Guild input", async () => {
    ...
    const response = await server.inject({
      method: "GET",
      url: "/api/v1/admin/bot-config?guildId=bad",
      headers: headers("l3"),
    });
  ...
  test("AT-CFG-003 rejects L3 Role validation and update at the API boundary", async () => {
    ...
    const preview = await validate(server, "l3", {
      staff_l4_role_id: "900000000000001201",
    });
    expect(preview.statusCode).toBe(403);
    const update = await server.inject({
      method: "PATCH",
      url: "/api/v1/admin/bot-config",
      headers: headers("l3", true),
      payload: {
        guildId,
        expectedVersion: 1,
        changes: { staff_l4_role_id: "900000000000001201" },
        reason: "Unauthorized Role change.",
        validationToken: "x".repeat(32),
      },
    });
  ...
  test("AT-CFG-005 returns a non-applicable preview for invalid Discord objects", async () => {
    ...
    const response = await validate(server, "l3", {
      gift_broadcast_channel_id: replacementChannelId,
    });
    expect(response.statusCode).toBe(200);
    expect(response.json().data).toMatchObject({
      mayApply: false,
    ...
    expect(response.json().data.errors[0]).toMatchObject({
      field: "gift_broadcast_channel_id",
      code: "MISSING_BOT_PERMISSION",
    });
    expect(store.snapshot(guildId)?.version).toBe(1);
  });
  ...
  test("AT-CFG-006 requires an unexpired preview token bound to actor, Guild, version, changes and reason", async () => {
    ...
    const missing = await server.inject({
      ...
      payload: {
        guildId,
        expectedVersion: 1,
        changes: { gift_broadcast_channel_id: replacementChannelId },
        reason: "Approved configuration change.",
        validationToken: "",
      },
    });
    ...
    const changed = await server.inject({
      ...
      payload: {
        guildId,
        expectedVersion: 1,
        changes: { gift_broadcast_channel_id: channelId },
        reason: "Approved configuration change.",
  ...
  test("AT-CFG-001/008 applies a previewed change, appends event/audit, and immediately serves the new value", async () => {
    ...
    const update = await server.inject({
      ...
      payload: {
        ...
        changes: { gift_broadcast_channel_id: replacementChannelId },
        reason: "Approved configuration change.",
        validationToken: preview.json().data.validationToken,
      },
    });
    ...
    expect(store.snapshot(guildId)?.values.gift_broadcast_channel_id).toBe(
    ...
    expect(store.events[0]).toMatchObject({
      guildId,
      version: 2,
      previousValues: { gift_broadcast_channel_id: channelId },
      actorStaffId: accounts.l3.staffId,
      source: "DISCORD_BOT",
    });
    expect(audit.records.at(-1)).toMatchObject({
      permissionCode: "bot_config.operational.manage",
    ...
    expect(current.json().data).toMatchObject({
      version: 2,
      values: { gift_broadcast_channel_id: replacementChannelId },
    });
  });

  test("AT-CFG-003 lets L4 replace a canonical staff Role mapping", async () => {
    const { server, store } = fixture();
    const preview = await validate(server, "l4", {
      staff_l4_role_id: replacementRoleId,
    });
    const update = await server.inject({
      method: "PATCH",
      url: "/api/v1/admin/bot-config",
      headers: headers("l4", true),
      payload: {
        guildId,
        expectedVersion: 1,
        changes: { staff_l4_role_id: replacementRoleId },
        reason: "Approved configuration change.",
        validationToken: preview.json().data.validationToken,
      },
    });
    ...
    expect(update.statusCode).toBe(200);
    expect(store.roleMapping(guildId, "staff_l4_role_id")).toBe(
      replacementRoleId,
    );
  });

  test("AT-CFG-001 uses the updated gift channel on the next real business action", async () => {
    const config = new InMemoryBotConfigStore({
      snapshots: [
        {
          ...initial,
          version: 2,
    ...
    const store = new InMemoryGiftStore({
      requests: [gift.request],
      reservations: [gift.reservation],
      staffTasks: [gift.task],
      externalUserIds: { [gift.request.senderId]: "mock-user-ok" },
      displayNames: {
        [gift.request.senderId]: "用户小林",
        [gift.request.receiverId]: "陪玩阿青",
      },
      guildIdsByOrder: { [gift.request.orderId]: guildId },
    });
    await captureApprovedGift({
      store,
      broadcastChannelId: channelId,
      botConfigStore: config,
      giftRequestId: gift.request.id,
      now,
    });

    expect(store.broadcasts[0]?.payload).toMatchObject({
  ...
  test("AT-CFG-007 rejects a stale preview without overwriting a newer value", async () => {
    const { server, store } = fixture();
    const stalePreview = await validate(server, "l3", {
      gift_broadcast_channel_id: replacementChannelId,
    });
    const l4Preview = await validate(server, "l4", {
    ...
    const first = await server.inject({
      ...
      payload: {
        guildId,
        expectedVersion: 1,
        changes: { maintenance_notice: true },
        reason: "Approved configuration change.",
        validationToken: l4Preview.json().data.validationToken,
      },
    });
    ...
    const stale = await server.inject({
      ...
      payload: {
        ...
        changes: { gift_broadcast_channel_id: replacementChannelId },
        reason: "Approved configuration change.",
        validationToken: stalePreview.json().data.validationToken,
      },
    });
    expect(stale.statusCode).toBe(409);
    expect(stale.json().error.code).toBe("CONFIG_VERSION_CONFLICT");
    expect(store.snapshot(guildId)?.values).toMatchObject({
      gift_broadcast_channel_id: channelId,
      maintenance_notice: true,
    });
  });
  ...
  test("AT-CFG-005 test delivery sends only a marked Discord test message", async () => {
    ...
    const response = await server.inject({
      ...
      payload: {
        ...
        channelId,
        reason: "Verify Discord delivery.",
      },
    });
    expect(response.statusCode).toBe(200);
    expect(response.json().data).toMatchObject({
...
function approvedGiftFixture(): {
  request: GiftRequestRecord;
  reservation: GiftReservationRecord;
  task: GiftStaffTaskRecord;
} {
  const request: GiftRequestRecord = {
    id: "00000000-0000-0000-0000-000000010100",
    publicId: "G-10100",
    orderId: "00000000-0000-0000-0000-000000010101",
    giftCatalogVersionId: "00000000-0000-0000-0000-000000010102",
    senderId: "00000000-0000-0000-0000-000000010103",
    receiverId: "00000000-0000-0000-0000-000000010104",
    status: "APPROVED",
    version: 3,
    ...
    priceMinor: 5000,
    currency: "CAT",
    broadcastTemplateSnapshot: "{sender_name} 送给 {receiver_name} {gift_name}",
    verifiedByStaffId: accounts.l3.staffId,
    verifiedAt: now.toISOString(),
    verificationNote: "confirmed",
    verificationPayloadHash: "hash",
    executionCredentialExpiresAt: new Date(
      now.getTime() + 15 * 60_000,
    ).toISOString(),
    approvedByStaffId: accounts.l3.staffId,
  ...
  const task: GiftStaffTaskRecord = {
    ...
    giftRequestId: request.id,
    claimedBy: accounts.l3.staffId,
    voiceChannelId: "900000000000001300",
    contextSnapshot: {
      orderId: request.orderId,
      orderPublicId: "P-10101",
      channelId: "900000000000001301",
      voiceChannelId: "900000000000001300",
      senderId: request.senderId,
      receiverId: request.receiverId,
      giftCode: request.giftCodeSnapshot,
      giftName: request.giftNameSnapshot,

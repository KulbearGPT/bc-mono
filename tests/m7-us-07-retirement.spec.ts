...
describe("M7-US-07 Provider funding retirement", () => {
  ...
  test("contains no current Provider funding, binding, recharge URL, or CNY runtime contract", async () => {
    const runtime = (
      await Promise.all(
        currentRuntimeFiles.map((file) => readFile(file, "utf8")),
      )
    ).join("\n");
    ...
    expect(runtime).not.toMatch(
      /providerBalanceMinor|PROVIDER_(?:TIMEOUT|UNAVAILABLE)|PAYMENT_PROVIDER_|RECHARGE_URL/u,
    );
    expect(runtime).not.toMatch(
      /binding-modal|modal:binding|\/api\/v1\/bindings|\/api\/v1\/webhooks\/payment/u,
    );
    expect(runtime).not.toMatch(
      /provider_balance_snapshots|JOIN external_accounts|FROM external_accounts/u,
    );
    expect(runtime).not.toMatch(
      /currency:\s*['"]CNY['"]|defaultValue=['"]CNY['"]/u,
    );
  });
  ...
  test("inventories every mutation route and production worker handler under the universal audit contract", async () => {
    ...
    for (const file of (await readdir("apps/api/src"))
      .filter((name) => name.endsWith(".ts") && name !== "security.ts")
      .sort()) {
      ...
      for (const match of source.matchAll(/registerSecureWriteRoute\s*\(/gu)) {
        ...
        routeInventory.push({
          id:
            `${file}:${requiredMatch(registration, /method\s*:\s*['"](POST|PUT|PATCH|DELETE)['"]/u)}` +
            `:${requiredMatch(registration, /url\s*:\s*['"]([^'"]+)['"]/u)}`,
          actorSources:
            registration
              .match(/acceptedSources\s*:\s*\[([^\]]+)\]/u)?.[1]
              ?.replaceAll(/\s+/gu, "") ?? "ALL_TRUSTED",
          primaryTarget: requiredMatch(
            registration,
            /targetType\s*:\s*['"]([^'"]+)['"]/u,
          ),
          action: requiredMatch(registration, /action\s*:\s*['"]([^'"]+)['"]/u),
          successChangeBuilder:
            "registerSecureWriteRoute:primary-or-explicit-audit-changes",
          failureRejectionAuditPath:
            "registerSecureWriteRoute:FAILED-or-REJECTED",
    ...
    const handlerMap = workerRuntime.slice(
      workerRuntime.indexOf("const handlers:"),
      workerRuntime.indexOf("return handlers;"),
    );
    const workerInventory = [
      ...handlerMap.matchAll(
        /(?:handlers\.)?([A-Z][A-Z_]+)\s*[:=]\s*input\./gu,
      ),
    ].map((match) => ({
      id: `worker:${match[1]}`,
    ...
    expect(routeInventory).toHaveLength(106);
    expect(workerInventory).toHaveLength(12);
    const inventory = [...routeInventory, ...workerInventory];
    expect(new Set(inventory.map(({ id }) => id)).size).toBe(inventory.length);
    for (const entry of inventory) {
      expect(
        Object.values(entry).every((value) => value.length > 0),
        entry.id,
      ).toBe(true);
...
function requiredMatch(source: string, pattern: RegExp): string {
  ...
  if (!value)
    throw new Error(`Mutation audit inventory could not resolve ${pattern}.`);
  return value;
}

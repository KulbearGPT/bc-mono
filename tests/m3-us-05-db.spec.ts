...
describe('M3-US-05 PostgreSQL private financial history', () => {
  ...
  test('persists an idempotent append-only reversal and keeps the original amount', async () => {
    ...
    const input = { commissionId, expectedVersion: 1, action: 'CREATE_REVERSAL' as const,
      reversalAmount: { amountMinor: 20, currency: 'USD' }, reason: 'partial refund',
...
async function seed() {
  await pool.query(`INSERT INTO users (id,display_name,status,row_version,created_at,updated_at) VALUES
    ...
    VALUES ('00000000-0000-0000-0000-000000004230','P-4230','${customerId}','${beneficiaryId}','COMPLETED',1,10000,'USD','${guildId}','2026-07-18T10:00:00Z','2026-07-18T09:00:00Z','2026-07-18T10:00:00Z');
    ...
    ('00000000-0000-0000-0000-000000004220','${customerId}','ORDER_CHARGE','DEBIT','00000000-0000-0000-0000-000000004230',10000,'USD','ORDER','00000000-0000-0000-0000-000000004230','history:order','2026-07-18T10:00:00Z'),
    ('00000000-0000-0000-0000-000000004221','${customerId}','GIFT_CHARGE','DEBIT','00000000-0000-0000-0000-000000004230',3000,'USD','GIFT','00000000-0000-0000-0000-000000004231','history:gift','2026-07-18T11:00:00Z'),
    ('00000000-0000-0000-0000-000000004222','${customerId}','REFUND_REVERSAL','CREDIT','00000000-0000-0000-0000-000000004230',1000,'USD','REFUND','00000000-0000-0000-0000-000000004232','history:refund','2026-07-18T12:00:00Z'),
    ('00000000-0000-0000-0000-000000004223','${beneficiaryId}','GIFT_CHARGE','DEBIT','00000000-0000-0000-0000-000000004230',3000,'USD','GIFT','00000000-0000-0000-0000-000000004233','history:other-gift','2026-07-18T12:30:00Z');
    ...
    VALUES ('00000000-0000-0000-0000-000000004240','PLAYER_LIFETIME',1,'ACTIVE','PLAYER_LIFETIME','NET_SPEND_BPS',200,'USD',true,true,'${staffId}',now(),now());
    ...
    ('${commissionId}','00000000-0000-0000-0000-000000004241','${beneficiaryId}','00000000-0000-0000-0000-000000004220','PLAYER_LIFETIME',1,'NET_SPEND_BPS',10000,200,200,'USD','PENDING',1,'2026-07-18T13:00:00Z','2026-07-18T13:00:00Z'),
    ('00000000-0000-0000-0000-000000004215','00000000-0000-0000-0000-000000004242','${otherBeneficiaryId}','00000000-0000-0000-0000-000000004223','PLAYER_LIFETIME',1,'NET_SPEND_BPS',3000,200,60,'USD','PENDING',1,'2026-07-18T14:00:00Z','2026-07-18T14:00:00Z');
    ...
    VALUES ('00000000-0000-0000-0000-000000004250','${commissionId}','REVERSAL_DEBIT',80,'USD','refund','commission:seed:adjustment','${staffId}',now());`);
}

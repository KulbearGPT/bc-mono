...
function order(): OrderRecord { return { id:orderId,publicId:'P-4312',customerId,playerId,status:'IN_SERVICE',version:7,serviceCatalogId:null,catalogVersion:null,
  ...
  amountMinor:12000,playerEarningMinor:8400,currency:'USD',notes:null,channelSpec:{channelId:'900000000000000003',panelMessageId:'900000000000000004',voiceChannelId:null},createdAt:now.toISOString(),updatedAt:now.toISOString() }; }
function request(status: GiftRequestRecord['status']='PENDING_REVIEW'): GiftRequestRecord { return { id:giftRequestId,publicId:'G-4313',orderId,giftCatalogVersionId:'00000000-0000-0000-0000-000000004314',senderId:customerId,receiverId:playerId,status,version:1,giftCodeSnapshot:'STAR',giftNameSnapshot:'星光礼盒',priceMinor:2000,currency:'USD',broadcastTemplateSnapshot:'gift',expiresAt:new Date(now.getTime()+30*60_000).toISOString(),createdAt:now.toISOString(),updatedAt:now.toISOString() }; }
function reservation(status: GiftReservationRecord['status']='ACTIVE'): GiftReservationRecord { return { id:'00000000-0000-0000-0000-000000004315',userId:customerId,sourceType:'GIFT',orderId:null,giftRequestId,mode:'LOCAL_RESERVATION',provider:'mock-provider',providerHoldRef:null,amountMinor:2000,currency:'USD',status,version:2,idempotencyKey:'gift:4313',expiresAt:request().expiresAt,activatedAt:now.toISOString(),settledAt:null,createdAt:now.toISOString(),updatedAt:now.toISOString() }; }
function task(): GiftStaffTaskRecord { return {id:'00000000-0000-0000-0000-000000004316',publicId:'T-4316',type:'GIFT_REVIEW',reasonCode:'GIFT_REQUESTED',status:'OPEN',version:1,orderId,giftRequestId,voiceChannelId:null,contextSnapshot:{orderId,orderPublicId:'P-4312',channelId:'900000000000000003',voiceChannelId:null,senderId:customerId,receiverId:playerId,giftCode:'STAR',giftName:'星光礼盒',priceMinor:2000,currency:'USD',reservationId:reservation().id},createdAt:now.toISOString(),updatedAt:now.toISOString()}; }

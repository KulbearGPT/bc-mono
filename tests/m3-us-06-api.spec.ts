...
import { InMemoryGiftStore, createGiftExpiryHandler, expireGiftRequest, registerGiftRoutes, type GiftApprovalRecord, type GiftRequestRecord, type GiftReservationRecord, type GiftStaffTaskRecord } from '@blackcat/api/gifts';
...
function order(): OrderRecord { return { id:orderId,publicId:'P-4312',customerId,playerId,status:'IN_SERVICE',version:7,serviceCatalogId:null,catalogVersion:null,
  ...
  amountMinor:12000,playerEarningMinor:8400,currency:'CAT',notes:null,channelSpec:{channelId:'900000000000000003',panelMessageId:'900000000000000004',voiceChannelId:null},createdAt:now.toISOString(),updatedAt:now.toISOString() }; }
function request(status: GiftRequestRecord['status']='PENDING_REVIEW'): GiftRequestRecord { return { id:giftRequestId,publicId:'G-4313',orderId,giftCatalogVersionId:'00000000-0000-0000-0000-000000004314',senderId:customerId,receiverId:playerId,status,version:1,giftCodeSnapshot:'STAR',giftNameSnapshot:'星光礼盒',priceMinor:2000,currency:'CAT',broadcastTemplateSnapshot:'gift',expiresAt:new Date(now.getTime()+30*60_000).toISOString(),createdAt:now.toISOString(),updatedAt:now.toISOString() }; }
function reservation(status: GiftReservationRecord['status']='ACTIVE'): GiftReservationRecord { return { id:'00000000-0000-0000-0000-000000004315',userId:customerId,sourceType:'GIFT',orderId:null,giftRequestId,mode:'LOCAL_RESERVATION',provider:'mock-provider',providerHoldRef:null,amountMinor:2000,currency:'CAT',status,version:2,idempotencyKey:'gift:4313',expiresAt:request().expiresAt,activatedAt:now.toISOString(),settledAt:null,createdAt:now.toISOString(),updatedAt:now.toISOString() }; }
function task(): GiftStaffTaskRecord { return {id:'00000000-0000-0000-0000-000000004316',publicId:'T-4316',type:'GIFT_REVIEW',reasonCode:'GIFT_REQUESTED',status:'OPEN',version:1,orderId,giftRequestId,voiceChannelId:null,contextSnapshot:{orderId,orderPublicId:'P-4312',channelId:'900000000000000003',voiceChannelId:null,senderId:customerId,receiverId:playerId,giftCode:'STAR',giftName:'星光礼盒',priceMinor:2000,currency:'CAT',reservationId:reservation().id},createdAt:now.toISOString(),updatedAt:now.toISOString()}; }
function approval():GiftApprovalRecord{return{id:'00000000-0000-0000-0000-000000004320',publicId:'APR-4320',action:'GIFT_APPROVE',targetId:giftRequestId,targetVersion:1,payloadSnapshot:{},payloadHash:'hash',amountMinor:2000,currency:'CAT',requestedByStaffId:'00000000-0000-0000-0000-000000004321',requiredLevel:'L3_OPERATIONS',reason:'review',expiresAt:new Date(now.getTime()+60*60_000).toISOString(),createdAt:now.toISOString(),status:'PENDING'};}
...
function fixture(status: GiftRequestRecord['status']='PENDING_REVIEW') { const store=new InMemoryGiftStore({requests:[request(status)],reservations:[reservation(status==='CAPTURED'?'CAPTURED':'ACTIVE')],staffTasks:[task()]});
  ...
  registerGiftRoutes(server,{store,orderStore:new InMemoryOrderStore({orders:[order()]}),accountStore:new InMemoryAccountStore({bindings:[binding()],reservationSource:()=>store.reservations}),walletFunding:new TestWalletFunding(),broadcastChannelId:'900000000000000020',now:()=>now});return{server,store}; }
...
describe('M3-US-06 gift reservation lifecycle',()=>{
  test('lets only the sender withdraw before capture and releases once',async()=>{const {server,store}=fixture();const input={method:'POST' as const,url:`/api/v1/gift-requests/${giftRequestId}/cancel`,headers:headers('gift:cancel:4313'),payload:{expectedVersion:1,reasonCode:'CUSTOMER_WITHDREW_REQUEST'}};
    store.approvals.push(approval());const first=await server.inject(input);const replay=await server.inject(input);expect(first.json()).toMatchObject({data:{status:'WITHDRAWN',reservation:{status:'RELEASED'}}});expect(replay.statusCode).toBe(200);expect(store.reservations[0]).toMatchObject({status:'RELEASED',version:3});expect(store.approvals[0]).toMatchObject({status:'CANCELLED'});
  ...
  test('expires and releases the same request idempotently',async()=>{const {store}=fixture();const at=new Date(Date.parse(request().expiresAt)+1);
    ...
    const replay=await expireGiftRequest({store,giftRequestId,now:at});expect(replay).toMatchObject({status:'EXPIRED',reservation:{status:'EXPIRED'}});expect(store.reservations[0]).toMatchObject({status:'EXPIRED',version:3});expect(store.approvals[0]).toMatchObject({status:'EXPIRED'});});
});

...
const summary={windowStart:'2026-07-18T16:00:00.000Z',windowEnd:'2026-07-19T16:00:00.000Z',timeZone:'Asia/Shanghai',currency:'USD',metrics:{todayOrderCount:42,inProgressOrderCount:11,pendingStaffTaskCount:8,completedOrderNetConsumptionMinor:842000,giftNetConsumptionMinor:193600,activeReservedMinor:216400,dispatchSuccessRateBps:9170,exceptionCount:3}};
...
describe('M4-US-09 Dashboard metric summary',()=>{
  test('renders all eight metrics with money and basis-point formatting',()=>{
    ...
    expect(html).toContain('USD\u00a08,420.00');expect(html).toContain('91.70%');expect(html).toContain('Asia/Shanghai');
  });

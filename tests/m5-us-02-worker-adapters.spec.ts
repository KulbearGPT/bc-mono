...
describe('M5-US-02 Worker production adapters', () => {
      ...
      rows: [
        {
          ...
          lifecycle_participants: projection.participants,
          readiness_due_at: projection.readyDeadlineAt,
          ...
          config_json: {
            private_order_category_id: projection.privateOrderCategoryId,
            staff_task_channel_id: projection.staffTaskChannelId,
            staff_l1_role_id: projection.staffRoleIds?.[0]
          }
        }
      ]
    });
  ...
  test('queues one idempotent rollout refresh for every active customer order panel', async () => {
    ...
    await expect(store.enqueuePanelExperienceNormalization(new Date('2026-08-10T08:00:00.000Z'))).resolves.toBe(1);
  ...
  test('distinguishes a missing order from an optimistic panel-id conflict', async () => {
    const missingQuery = vi.fn().mockResolvedValueOnce({ rows: [] }).mockResolvedValueOnce({ rows: [] });
    const missing = new PostgresOrderPanelProjectionStore({ query: missingQuery });
    await expect(
      missing.replacePanelMessageId({
        orderId: projection.orderId,
        expectedPanelMessageId: projection.panelMessageId,
        panelMessageId: '530000000000000005'
      })
    ...
    await expect(
      conflict.replacePanelMessageId({
        orderId: projection.orderId,
        expectedPanelMessageId: projection.panelMessageId,
        panelMessageId: '530000000000000005'
      })
  ...
  test('grants the assigned player access and patches the existing panel message', async () => {
    ...
    const panelButtons = panelBody.components[0].components
      .filter((component: { type: number }) => component.type === 1)
      .flatMap((component: { components?: unknown[] }) => component.components ?? []);
    expect(panelButtons).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ custom_id: `bc:service:support:${projection.orderId}:v8` }),
        expect.objectContaining({ label: '刷新最新状态', custom_id: `bc:order:${projection.orderId}:refresh` }),
        expect.objectContaining({ label: '申请取消订单', custom_id: `bc:order:${projection.orderId}:cancel:v8` })
      ])
    );
    expect(panelButtons).not.toEqual(
      expect.arrayContaining([
  ...
  test('keeps recovery controls when synchronizing a pending-dispatch panel', async () => {
    ...
    expect(buttons).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ label: '开始招募陪玩', custom_id: `bc:sp:new:${projection.orderId}:o8` }),
        expect.objectContaining({ label: '刷新最新状态', custom_id: `bc:order:${projection.orderId}:refresh` }),
  ...
  test('renders current collecting facts and the close control when a selection pool panel is synchronized', async () => {
    const pending = {
      ...
      selectionPool: {
        ...
        round: 2,
        applicationCount: 3,
        applicantDiscordUserIds: ['530000000000000021', '530000000000000022', '530000000000000023'],
        closesAt: null
      }
    ...
    expect(JSON.stringify(body.components)).toContain('<@530000000000000021>');
    expect(JSON.stringify(body.components)).toContain('<@530000000000000023>');
    const actionRows = body.components[0].components.filter((component: { type: number }) => component.type === 1);
    expect(actionRows.flatMap((row: { components: unknown[] }) => row.components)).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ label: '结束报名，进入试音', custom_id: expect.stringMatching(/^bc:sp:c:/u) }),
        expect.objectContaining({ label: '刷新最新状态' }),
        expect.objectContaining({ label: '取消订单' })
      ])
    );
  });

  test('restores one restart button after an empty selection round closes', async () => {
    const pending = {
      ...projection,
      status: 'PENDING_DISPATCH',
      playerDiscordUserId: null,
    ...
    const body = JSON.parse(fetchMock.mock.calls[0]?.[1]?.body as string);
    expect(JSON.stringify(body.components)).toContain('本轮无人报名');
    const button = body.components[0].components
      .flatMap((component: { components?: unknown[] }) => component.components ?? [])
      .find(
        (component: { type?: number; custom_id?: string }) =>
          component.type === 2 && component.custom_id?.startsWith('bc:sp:r:')
      );
    expect(button).toMatchObject({ label: '再发起一轮报名' });
  });
  ...
  test('creates one private voice room and sends idempotent customer and staff coordination notices after acceptance', async () => {
    ...
    expect(createVoice.permission_overwrites).toEqual(
      expect.arrayContaining([
    ...
    expect(staffNotice.embeds).toEqual([
      expect.objectContaining({
        title: '🛠️ 新订单协调 · P-5301',
        description: expect.stringContaining('已匹配完成'),
        fields: expect.arrayContaining([
          { name: '当前状态', value: '等待陪玩全员准备（ACCEPTED）', inline: true },
  ...
  test.each([
    ...
    const staffPatch = fetchMock.mock.calls.find(
      ([url, init]) =>
  ...
  test('posts a replacement only when the existing panel PATCH returns 404', async () => {
    ...
    await expect(
      adapter.upsertOrderPanel(
        { ...projection, playerDiscordUserId: null, playerDiscordUserIds: [], filledPlayerCount: 0 },
        notBefore
      )
    ).resolves.toEqual({
    ...
    expect(fetchMock.mock.calls[1]?.[0]).toBe(
      `https://discord.com/api/v10/channels/${projection.channelId}/messages?limit=100`
    );
  ...
  test('exposes Discord retry_after when panel delivery is rate limited', async () => {
    const fetchMock = vi.fn().mockResolvedValue(
      new Response(JSON.stringify({ retry_after: 0.5 }), {
        status: 429,

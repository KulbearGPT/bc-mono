...
describe('M2-US-11 Discord paused automation state', () => {
  ...
  test('marks the active order as staff-managed in the private service center', () => {
    const message = buildServiceCenterMessage({
      ...
      balance: { providerBalanceMinor: 20000, reservedMinor: 12000, availableMinor: 8000, currency: 'CNY', fetchedAt: '2026-07-18T09:00:00.000Z' },
      activeOrder: pausedOrder(), consumptions: { items: [], nextCursor: null },
      commissions: { summary: { pendingMinor: 0, confirmedMinor: 0, paidMinor: 0, currency: 'CNY' }, items: [], nextCursor: null }
    });
...
function pausedOrder(): OrderSummary {
  return {
    ...
    amountMinor: 12000, currency: 'CNY', notes: null,

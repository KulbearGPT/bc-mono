...
describe('M2-US-11 Discord paused automation state', () => {
  test('shows staff takeover and removes controls that advance automation', () => {
    ...
    expect(controls).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ type: 'BUTTON', label: '联系猫舍前台' }),
        expect.objectContaining({ type: 'BUTTON', label: '取消订单' })
      ])
    );
  ...
  test('marks the active order as staff-managed in the private service center', () => {
    const message = buildServiceCenterMessage({
      ...
      balance: {
        ledgerBalanceMinor: 20000,
        reservedMinor: 12000,
        availableMinor: 8000,
        currency: 'CAT',
        calculatedAt: '2026-07-18T09:00:00.000Z'
      },
      activeOrder: pausedOrder(),
...
function pausedOrder(): OrderSummary {
  return {
    id: '00000000-0000-0000-0000-00000000b611',
    publicId: 'P-611',
    ...
    matching: {
      ...
      timeoutAt: '2026-07-18T09:05:00.000Z',

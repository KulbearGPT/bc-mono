...
function offer(overrides: Partial<DispatchOfferSummary> = {}): DispatchOfferSummary {
  return {
    dispatchAttemptId,
    orderId,
...
describe('M2-US-02 Bot dispatch card', () => {
  test('renders a concentrated dispatch offer card with accept and decline actions', () => {
    ...
    expect(message.body).toContain('预计收益：¥84.00');
  ...
  test('HttpBotApiClient accepts and declines dispatch offers through the unified API', async () => {
    const fetchMock = vi.fn()
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ data: { id: orderId, status: 'ACCEPTED', version: 4 } })
      })
      .mockResolvedValueOnce({
        ok: true,
        status: 200,
        json: async () => ({ data: { id: orderId, status: 'PENDING_DISPATCH', version: 3 } })
      });
  ...
  test('registers a Sapphire dispatch interaction handler', async () => {
    ...
    expect(manifest.pieces).toEqual(
      expect.arrayContaining([
        expect.objectContaining({ kind: 'interaction-handlers', name: 'dispatch-buttons' })
      ])
    );
    expect(source).toContain('acceptOrder');
    expect(source).toContain('declineOrderOffer');

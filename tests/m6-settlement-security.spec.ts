...
function earning(id: string, playerUserId: string, guildId: string): SettlementCandidateEarning {
  return {
    id, orderId: id, playerUserId, guildId, amountMinor: 10_000, currency: 'CNY', status: 'CONFIRMED',
...
describe('settlement security remediation', () => {
  ...
  test('requires and atomically creates a replacement when voiding an approved batch', async () => {
    ...
    expect(store.get(guildA, replacementBatchId)).toMatchObject({ guildId: guildA, currency: 'CNY', status: 'DRAFT' });
  });

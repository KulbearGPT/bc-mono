...
describe('M4-US-03 PostgreSQL admin directory', () => {
  ...
  test('rolls back all three admin mutations when the transactional audit insert fails', async () => {
    ...
    const createWrite = await store.createGiftCatalog({ name: 'Bouquet', amountMinor: 1800, currency: 'CNY', enabled: true,
...
async function seed() {
  await pool.query(`INSERT INTO users (id,display_name,status,row_version,created_at,updated_at) VALUES
    ...
    VALUES ('${ids.order}','P-4305','${ids.customer}','${ids.player}','${ids.customer}','${ids.player}','IN_SERVICE',2,'CNY',12000,'900000000000004300','900000000000004303','900000000000004304',now(),now());
    ...
    VALUES ('00000000-0000-0000-0000-000000004314','${ids.customer}','ORDER_CHARGE','ORDER','${ids.order}','m4:admin:order:4305','${ids.order}',12000,'CNY','DEBIT',now(),now()),
      ('00000000-0000-0000-0000-000000004318','${ids.customer}','ADMIN_CORRECTION','ADMIN','00000000-0000-0000-0000-000000004319','m4:admin:correction:4318',NULL,300,'CNY','CREDIT',now(),now());
    ...
    VALUES ('00000000-0000-0000-0000-000000004315','${ids.gift}',1,'ACTIVE','${ids.gift}','Rocket',5200,'CNY','{sender} sent {gift}','${ids.staff}',now(),now());
    ...
    VALUES ('${ids.giftRequest}','G-4307','${ids.order}','00000000-0000-0000-0000-000000004315','${ids.customer}','${ids.player}','CAPTURED',4,'ROCKET','Rocket',5200,'CNY','{sender} sent {gift}',now(),now() + interval '1 hour',now(),now());

...
describe('M2-US-08 player workbench API', () => {
  test('returns qualification, separate presence and availability, matching requirements, countdown, own earnings and API capabilities', async () => {
    const store = new InMemoryPlayerStore({ profiles: [profile], workbenches: { [playerUserId]: workbench } });
    const server = buildApiServer({
      env: {
        NODE_ENV: 'development', DATABASE_URL: '', API_PORT: '0', API_BASE_URL: 'http://localhost:3000', BOT_SERVICE_TOKEN: 'valid-bot-token'
      },
    ...
    const response = await server.inject({ method: 'GET', url: '/api/v1/players/me/workbench', headers: headers() });
    ...
    expect(response.json()).toMatchObject({
      data: {
        ...
        currentOrder: null,
        matchingOrders: [{
          dispatchAttemptId: workbench.matchingOrders[0]!.dispatchAttemptId,
          secondsRemaining: 120,
        ...
        earningsSummary: { pendingMinor: 8_000, confirmedMinor: 3_000, paidMinor: 20_000 },
        nextActions: ['REVIEW_MATCH', 'ACCEPT_ORDER']
      }
    });
    expect(JSON.stringify(response.json())).not.toMatch(/customerId|customerNote|externalAccount|phone|email/i);
  });
  ...
  test('does not offer dispatch actions when the player is unavailable', async () => {
    ...
    expect(response.json().data.nextActions).toEqual(['SET_AVAILABLE']);
  });
});

...
test.describe('Dashboard browser E2E: settlements and weekly reports', () => {
  ...
  test('DE2E-SET-008 voiding an approved batch requires and records a valid replacement batch', async ({ page, request }) => {
    await login(page, 'l4'); await stepUp(page, request, 'l4'); const first = await createBatch(page, false, 'void-a'); const originalId = first.body.data.id; await submitBatch(page, originalId, 1, 'void-a'); await approveBatch(page, originalId, 2, 'void-a');await page.getByRole('link',{name:'结算',exact:true}).click();await expect(page.getByText('APPROVED',{exact:true})).toBeVisible();await page.getByRole('button',{name:'作废',exact:true}).click();await page.getByLabel('确认创建替代批次并保留原批次审计链').check();await page.getByRole('button',{name:'确认作废'}).click();await expect(page.getByText('VOID',{exact:true})).toBeVisible();const state = await (await request.get('http://127.0.0.1:3000/__e2e/state')).json();expect(state.settlementBatches).toHaveLength(2);expect(state.settlementBatches[0]).toMatchObject({ status: 'VOID', replacementBatchId: state.settlementBatches[1].id });
  });

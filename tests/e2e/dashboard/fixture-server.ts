...
const initialStaffAccounts=[{staffId:owner.staffId,displayName:'店长 Alice',effectiveLevel:'L4_ADMIN_OWNER',pendingElevationLevel:null,permissionsVersion:owner.permissionsVersion,activeSessions:1,status:'ACTIVE',roleSyncedAt:'2026-08-10T11:55:00.000Z',observedDiscordRoleIds:['role-e2e-l4'],lastRoleSyncStatus:'APPLIED',roleSyncQueueStatus:'COMPLETED',lastRoleSyncError:null},{staffId:'00000000-0000-0000-0000-000000000117',displayName:'客服小白',effectiveLevel:'L2_SUPERVISOR',pendingElevationLevel:'L3_OPERATIONS',permissionsVersion:4,activeSessions:2,status:'ACTIVE',roleSyncedAt:'2026-08-10T11:50:00.000Z',observedDiscordRoleIds:['role-e2e-l2','role-e2e-l3'],lastRoleSyncStatus:'PENDING_ELEVATION',roleSyncQueueStatus:'FAILED',lastRoleSyncError:'Discord API timeout'}] as Array<{staffId:string;displayName:string;effectiveLevel:'L1_SUPPORT'|'L2_SUPERVISOR'|'L3_OPERATIONS'|'L4_ADMIN_OWNER';pendingElevationLevel:'L1_SUPPORT'|'L2_SUPERVISOR'|'L3_OPERATIONS'|'L4_ADMIN_OWNER'|null;permissionsVersion:number;activeSessions:number;status:'ACTIVE'|'REVOKED';roleSyncedAt:string|null;observedDiscordRoleIds:string[];lastRoleSyncStatus:string|null;roleSyncQueueStatus:'PENDING'|'PROCESSING'|'COMPLETED'|'FAILED'|null;lastRoleSyncError:string|null}>;
...
registerSecureWriteRoute(server,server.securityOptions!,{method:'POST',url:'/api/v1/admin/staff/:staffId/discord-role-reconcile',permission:'access.manage',action:'RECONCILE_E2E_STAFF_DISCORD_ROLE',targetType:'staff_account',acceptedSources:['DASHBOARD'],requiresRecentStepUp:true,successStatusCode:202,
  handler:(request)=>{const target=staffAccounts.find((item)=>item.staffId===(request.params as {staffId:string}).staffId);if(!target)throw new Error('MISSING_STAFF');target.roleSyncQueueStatus='PENDING';target.lastRoleSyncError=null;return{staffId:target.staffId,jobId:'00000000-0000-0000-0000-000000000991',status:'QUEUED'};}});
registerSecureWriteRoute(server, server.securityOptions!, {
...
registerSecureWriteRoute(server, server.securityOptions!, {
  ...
  handler: (request,actor) => { const batch = settlementBatches.find((item) => item.id === (request.params as { batchId: string }).batchId); const body = request.body as { expectedVersion?: unknown; replacementBatchId?: unknown; replacement?: { guildId?: unknown;source?:unknown;periodStart?:unknown;periodEnd?:unknown;timeZone?:unknown; currency?: unknown } }; let replacement = settlementBatches.find((item) => item.id === body.replacementBatchId); if (!batch || batch.version !== body.expectedVersion || !['APPROVED', 'EXPORTED'].includes(String(batch.status))) throw new Error('STALE_SETTLEMENT');if(!replacement&&typeof body.replacementBatchId==='string'&&body.replacement?.source==='MANUAL'&&typeof body.replacement.periodStart==='string'&&typeof body.replacement.periodEnd==='string'&&typeof body.replacement.timeZone==='string'&&body.replacement.currency===batch.currency){const index=settlementBatches.length+1;replacement={id:body.replacementBatchId,publicId:`S-E2E-${String(index).padStart(3,'0')}`,status:'DRAFT',periodStart:body.replacement.periodStart,periodEnd:body.replacement.periodEnd,cutoffAt:body.replacement.periodEnd,timeZone:body.replacement.timeZone,currency:body.replacement.currency,netAmountMinor:4000,version:1,createdByStaffId:actor.actorStaffId,approvedByStaffId:null,sourceLocked:false,replacementBatchId:null,items:[]};settlementBatches.push(replacement);}if (!replacement || replacement.id === batch.id || replacement.currency !== batch.currency || (body.replacement?.guildId!==undefined&&body.replacement.guildId !== guildId) || body.replacement?.currency !== batch.currency || replacement.replacementBatchId === batch.id) throw new Error('INVALID_REPLACEMENT'); batch.status = 'VOID'; batch.replacementBatchId = replacement.id; batch.version = Number(batch.version) + 1; return { ...batch }; }
});
registerSecureReadRoute(server, server.securityOptions!, {

...
function buildPlayerServerThroughApiOptions(records: PlayerProfileRecord[] = [player()]) {
  const auditSink = new InMemoryAuditSink();
  const idempotencyStore = new InMemoryIdempotencyStore();
  const store = new InMemoryPlayerStore({ profiles: records });
  const server = buildApiServer({
...
describe('M2-US-01 player eligibility domain', () => {
  test('candidate selection requires active review, availability, online presence, matching tags, active user and no active order', () => {
    const eligible = player({ playerId: '00000000-0000-0000-0000-00000000a001', userId: '00000000-0000-0000-0000-00000000u001'.replace('u', 'a') });
    const candidates = selectEligibleDispatchCandidates(
      [
        eligible,
        player({ playerId: '00000000-0000-0000-0000-00000000a002', reviewStatus: 'PENDING_REVIEW' }),
        player({ playerId: '00000000-0000-0000-0000-00000000a003', availability: 'BUSY' }),
        player({ playerId: '00000000-0000-0000-0000-00000000a004', discordPresence: 'IDLE' }),
        player({ playerId: '00000000-0000-0000-0000-00000000a005', serviceTags: ['RANKED'] }),
        player({ playerId: '00000000-0000-0000-0000-00000000a006', userStatus: 'PAUSED' }),
        player({ playerId: '00000000-0000-0000-0000-00000000a007', activeOrderId: '00000000-0000-0000-0000-00000000b777' })
      ],
      { game: 'VALORANT', service: 'ENTERTAINMENT' }
    );

    expect(candidates.map((candidate) => candidate.playerId)).toEqual(['00000000-0000-0000-0000-00000000a001']);
  });
});

describe('M2-US-01 player API contract', () => {
  test('buildApiServer mounts player routes from the player option for real runtime composition', async () => {
    const { server } = buildPlayerServerThroughApiOptions();

    const response = await server.inject({
      method: 'PUT',
      url: '/api/v1/players/me/availability',
      headers: botHeaders('111111111111111111', { 'idempotency-key': 'discord:player:availability:mounted' }),
  ...
  test('setPlayerAvailability updates only an ACTIVE current player business switch and leaves Discord presence independent', async () => {
    ...
    const response = await server.inject({
      method: 'PUT',
      url: '/api/v1/players/me/availability',
      headers: botHeaders('111111111111111111', { 'idempotency-key': 'discord:player:availability:busy' }),
      payload: { expectedVersion: 5, availability: 'BUSY' }
    });
    ...
    expect(store.profiles[0]).toMatchObject({ availability: 'BUSY', discordPresence: 'ONLINE', version: 6 });
    expect(auditSink.records.at(-1)).toMatchObject({ permissionCode: 'player.availability.manage_self', outcome: 'SUCCEEDED' });
  });

  test('setPlayerAvailability rejects non-active reviewed players with a clear eligibility error', async () => {
    const { server } = buildPlayerServer([player({ reviewStatus: 'PAUSED' })]);

    const response = await server.inject({
      method: 'PUT',
      url: '/api/v1/players/me/availability',
      headers: botHeaders('111111111111111111', { 'idempotency-key': 'discord:player:availability:paused' }),
      payload: { expectedVersion: 5, availability: 'AVAILABLE' }
    });

    expect(response.statusCode).toBe(422);
    expect(response.json()).toMatchObject({ error: { code: 'PLAYER_NOT_ELIGIBLE' } });
  });

  test('syncDiscordPresence records the presence signal without changing business availability', async () => {
    const { server, store } = buildPlayerServer([player({ availability: 'BUSY', discordPresence: 'OFFLINE' })]);

    const response = await server.inject({
      method: 'POST',
      url: '/api/v1/internal/discord/presence',
      headers: botHeaders('999999999999999999', { 'idempotency-key': 'discord:presence:111-online' }),
      payload: {
        guildId,
        discordUserId: '111111111111111111',
        presence: 'ONLINE',
    ...
    expect(store.profiles[0]).toMatchObject({ availability: 'BUSY', discordPresence: 'ONLINE' });
  });

  test('admin player approval requires L3 while L2 can update tags after approval', async () => {
    const { server, store } = buildPlayerServer([player({ reviewStatus: 'PENDING_REVIEW', availability: 'OFFLINE', gameTags: [], serviceTags: [], version: 1 })]);

    const denied = await server.inject({
      method: 'POST',
      url: '/api/v1/admin/players/00000000-0000-0000-0000-00000000a001/approve',
      headers: dashboardHeaders('222222222222222222', { 'idempotency-key': 'dashboard:player:approve:l2-denied' }),
      payload: { expectedVersion: 1, gameTags: ['VALORANT'], serviceTags: ['ENTERTAINMENT'], reasonCode: 'REVIEW_PASSED' }
    });
    const approved = await server.inject({
      method: 'POST',
      url: '/api/v1/admin/players/00000000-0000-0000-0000-00000000a001/approve',
      headers: dashboardHeaders('333333333333333333', { 'idempotency-key': 'dashboard:player:approve:l3' }),
      payload: { expectedVersion: 1, gameTags: ['VALORANT'], serviceTags: ['ENTERTAINMENT'], reasonCode: 'REVIEW_PASSED' }
    });
    const tagged = await server.inject({
      method: 'PUT',
      url: '/api/v1/admin/players/00000000-0000-0000-0000-00000000a001/tags',
      headers: dashboardHeaders('222222222222222222', { 'idempotency-key': 'dashboard:player:tags:l2' }),
      payload: { expectedVersion: 2, gameTags: ['VALORANT'], serviceTags: ['ENTERTAINMENT', 'COACHING'], reasonCode: 'SKILL_REVIEW_UPDATED' }
    });
    ...
    expect(store.profiles[0]).toMatchObject({ reviewStatus: 'ACTIVE', version: 3 });
  });
  ...
  test('setPlayerOperationalStatus pauses a player without changing Discord presence or self availability', async () => {
    ...
    const response = await server.inject({
      ...
      headers: dashboardHeaders('333333333333333333', { 'idempotency-key': 'dashboard:player:pause:l3' }),
      payload: { expectedVersion: 5, reviewStatus: 'PAUSED', reasonCode: 'PLAYER_REQUEST', note: 'temporary pause' }
    });
    ...
    expect(store.profiles[0]).toMatchObject({ reviewStatus: 'PAUSED', availability: 'AVAILABLE', discordPresence: 'ONLINE' });
  });
});

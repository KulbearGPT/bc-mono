...
describe('M6-US-04 internal wallet customer profile',()=>{
  test('returns wallet balance and server-side statistics without Provider fallback fields',async()=>{
    ...
    await wallet.createTopUp({userId,amountMinor:50_000,paymentChannel:'cash',externalTransactionId:'m6_04',paidAt:now.toISOString(),note:'verified',idempotencyKey:'m6:04:topup',actorStaffId:'00000000-0000-0000-0000-000000006402',actorLevel:'L1_SUPPORT',now});
    ...
    expect(result.balance).toMatchObject({ledgerBalanceMinor:50_000,reservedMinor:0,availableMinor:50_000,currency:'USD'});

...
describe('M10-US-09 game-scoped ordering API', () => {
  ...
  test('rejects mixed-game package versions and preserves the stable package game', () => {
    const store = packages();
    expect(() => store.createAdmin({actorStaffId:'staff',payload:{code:'LOL_DUO',displayName:'混合套餐',description:'不允许跨游戏',defaultCustomerPriceMinor:null,currency:'CAT',activate:false,slots:[{serviceCatalogVersionId:'00000000-0000-0000-0000-000000109101',unitCount:1,customerNoteTemplate:null},{serviceCatalogVersionId:'00000000-0000-0000-0000-000000109201',unitCount:1,customerNoteTemplate:null}],reasonCode:'CREATE_PACKAGE'},now:new Date()})).toThrowError(ServicePackageError);
    expect(() => store.createAdmin({actorStaffId:'staff',payload:{code:'LOL_DUO',displayName:'错误新版本',description:'稳定套餐不能改游戏',defaultCustomerPriceMinor:null,currency:'CAT',activate:false,slots:[{serviceCatalogVersionId:'00000000-0000-0000-0000-000000109201',unitCount:1,customerNoteTemplate:null}],reasonCode:'CREATE_PACKAGE'},now:new Date()})).toThrowError(ServicePackageError);
  });
  ...
  test('rejects adding a cross-game single item to an existing game basket',()=>{const requirement=requirementRecord();const store=new InMemoryOrderRequirementStore({orders:[{id:orderId,guildId,customerDiscordUserId,status:'DRAFT',version:1,amountMinor:30,sourcePackageVersionId:null,compositionMode:null}],requirements:[requirement],catalogs:[catalog('00000000-0000-0000-0000-000000109202','VALORANT','瓦洛兰特')]});expect(()=>store.add({orderId,actorGuildId:guildId,actorDiscordUserId:customerDiscordUserId,expectedOrderVersion:1,serviceCatalogVersionId:'00000000-0000-0000-0000-000000109202',unitCount:1,requestedPlayerCount:1,idempotencyKey:'cross-game-add',now:new Date()})).toThrowError(/selected game/i);expect(store.orders[0]).toMatchObject({version:1,amountMinor:30});expect(store.requirements).toEqual([requirement]);});
});
...
function pkg(id:string,code:string,game:string,gameDisplayName:string,catalogId:string):ServicePackageVersionRecord{return{id,code,version:1,status:'ACTIVE',game,gameDisplayName,displayName:code,description:code,defaultCustomerPriceMinor:null,currency:'CAT',slots:[{id:`${id.slice(0,-1)}1`,position:1,serviceCatalogVersionId:catalogId,game,gameDisplayName,service:'FUN',serviceDisplayName:'娱乐陪玩',region:null,regionDisplayName:null,billingUnitMinutes:60,unitCount:1,customerUnitPriceMinor:30,customerNoteTemplate:null}]};}

...
describe('M6-US-04 Dashboard settlement and profiles', () => {
  ...
  test('maps preview, create, review, export and payment result commands to unified API', () => {
    ...
    expect(buildSettlementRequest({ action: 'PAYMENT_RESULTS', batchId: 'batch-1', version: 3, fields: { results: [{ settlementItemId: 'item-1', expectedVersion: 1,
      result: 'FAILED', amountMinor: 0, currency: 'CNY', externalBatchReference: '', note: 'Provider rejected row' }] } })).toMatchObject({
  ...
  test('builds independent Profile requests and keeps a balance failure from hiding statistics or orders', () => {
    ...
    const modules: CustomerProfileModules = {
      ...
      balance: { kind: 'ERROR', requestId: 'req_provider_timeout', data: { providerBalanceMinor: 8_000, reservedMinor: 12_000, availableMinor: -4_000, currency: 'CNY', fetchedAt: '2026-07-18T10:00:00.000Z', stale: true } },
      statistics: { kind: 'READY', data: { orderCount: 3, completedOrderCount: 2, cancelledOrderCount: 1, refundCount: 1, orderSpendMinor: 30_001,
        giftSpendMinor: 2_500, refundMinor: 1_500, totalConsumptionMinor: 31_001, averageOrderAmountMinor: 15_000, currency: 'CNY' } },
      orders: { kind: 'READY', items: [{ id: 'order-1', publicId: 'P-1', status: 'COMPLETED', serviceKey: 'RANKED', playerDisplayName: '陪玩甲', amountMinor: 10_001, currency: 'CNY', createdAt: '2026-07-18T12:00:00.000Z' }], nextCursor: null },
  ...
  test('renders a balance error with no snapshot without hiding ready Profile modules', () => {
    const modules: CustomerProfileModules = {
      ...
      statistics: { kind: 'READY', data: { orderCount: 2, completedOrderCount: 1, cancelledOrderCount: 0, refundCount: 0,
        orderSpendMinor: 10_000, giftSpendMinor: 0, totalConsumptionMinor: 10_000, averageOrderAmountMinor: 10_000, currency: 'CNY' } },
      orders: { kind: 'READY', items: [{ id: 'order-2', publicId: 'P-NO-SNAPSHOT', status: 'COMPLETED', amountMinor: 10_000, currency: 'CNY' }], nextCursor: null },

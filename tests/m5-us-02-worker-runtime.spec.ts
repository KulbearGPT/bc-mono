...
describe("M5-US-02 production Worker runtime", () => {
  test("registers every production delivery job type", () => {
    ...
    const handlers = createProductionHandlerMap({
      ...
      giftExpiry: handler,
      selectionPoolClose: handler,
      selectionPoolSync: handler,
  ...
  test("recovers a stale processing job on startup and preserves its dedupe key", async () => {
    const job = fixtureJob({
      status: "PROCESSING",
      attempts: 1,
      lockedAt: new Date(now.getTime() - 120_000).toISOString(),
    ...
    const runtime = new ProductionOutboxRuntime({
      ...
      worker,
      handlers: {
        GIFT_ANNOUNCEMENT: async (claimed) => observed.push(claimed.dedupeKey),
      },
      now: () => now,
      staleLockMs: 60_000,
    });
  ...
  test("uses an external retry-after delay instead of the default backoff", async () => {
    ...
    const retryAfter = Object.assign(new Error("Discord rate limited."), {
  ...
  test("rebuilds a deleted order panel from the database projection and persists the replacement message id", async () => {
    ...
    const handler = createPanelSyncHandler({
      store: {
        getOrderPanelProjection: async () => ({
          ...
          version: 8,
          channelId: "channel-1",
          panelMessageId: "deleted-message",
          customerDiscordUserId: "customer-1",
          playerDiscordUserId: "player-1",
          amountMinor: 12_000,
          currency: "CAT",
        }),
    ...
    await handler(
      fixtureJob({
        ...
        aggregateType: "order",
        aggregateId: "order-1",
        payload: { orderId: "order-1", kind: "RECOVERY" },
      }),
    );
    ...
    expect(writes).toEqual([
      {
        orderId: "order-1",
        expectedPanelMessageId: "deleted-message",
        panelMessageId: "replacement-message",
      },
    ]);
  });
  ...
  test("reuses a persisted dispatch message id after a retry instead of creating another offer", async () => {
    ...
    const handler = createDispatchMessageHandler({
      store: {
        ...
        saveDispatchMessageId: async ({ messageId }) => {
      ...
      discord: {
        upsertDispatchOffer: async (_payload, existingMessageId) => {
          ...
          return {
            messageId: existingMessageId ?? "dispatch-message-1",
            recreated: existingMessageId === null,
          };
        },
      },
    });
  ...
  test("reuses the previous round message for the same order instead of posting another offer", async () => {
    ...
    const handler = createDispatchMessageHandler({
      store: {
        getReusableDispatchMessageId: async ({ orderId }) =>
          persisted.orderId === orderId ? persisted.messageId : null,
        saveDispatchMessageId: async (input) => {
          persisted = {
      ...
      discord: {
        upsertDispatchOffer: async (_payload, existingMessageId) => {
          calls.push(existingMessageId);
          return {
            messageId: existingMessageId ?? "unexpected-new-message",
            recreated: false,
          };
        },
      },
    ...
    expect(replacements).toEqual([
      {
        dispatchAttemptId: "attempt-1",
        orderId: "order-1",
        previousMessageId: "deleted-message",
        messageId: "replacement-message",
      },
    ]);
  });
  ...
  test("routes timeout, archive, and role reconciliation jobs only from validated payloads", async () => {
    const calls: string[] = [];
    await createDispatchTimeoutHandler({
      expire: async (id) => {
      ...
      fixtureJob({
        ...
        aggregateId: "attempt-2",
        payload: { dispatchAttemptId: "attempt-2", orderId: "order-2" },
      }),
    );
    await createReadinessTimeoutHandler({
      expire: async (job) => {
        calls.push(`readiness:${job.aggregateId}`);
      },
    })(
      fixtureJob({
        ...
        aggregateType: "order",
        aggregateId: "order-2",
        payload: { orderId: "order-2", readinessDueAt: now.toISOString() },
      }),
    );
    await createChannelArchiveHandler({
      archive: async (channelId) => {
        calls.push(`archive:${channelId}`);
      },
    })(
      fixtureJob({
        ...
        aggregateType: "order",
        aggregateId: "order-2",
        payload: { orderId: "order-2", channelId: "channel-2" },
      }),
    );
    await createRoleReconciliationHandler({
      ...
      fixtureJob({
        ...
        aggregateType: "discord_role_mapping",
        aggregateId: "mapping-2",
        payload: {
          guildId: "guild-2",
          mappingVersion: 4,
...
function fixtureJob(overrides: Partial<OutboxJob> = {}): OutboxJob {
  return {
    id: "00000000-0000-0000-0000-000000005201",
    type: "GIFT_ANNOUNCEMENT",
    status: "PENDING",
    payload: {},
    aggregateType: "gift_request",
    aggregateId: "00000000-0000-0000-0000-000000005202",
    dedupeKey: "gift:announcement:5202:v1",
    attempts: 0,
    maxAttempts: 8,

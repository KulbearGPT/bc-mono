...
describe('M7-US-05 wallet funding lifecycle',()=>{
  test('reserves, captures once, releases without debit, and credits business refund without Provider calls',async()=>{
    ...
    await wallet.createTopUp({userId,amountMinor:1_000_000,paymentChannel:'ZELLE',externalTransactionId:'pi_m7_05',paidAt:now.toISOString(),
    ...
    expect(order.balance).toMatchObject({ledgerBalanceMinor:1_000_000,reservedMinor:120_000,availableMinor:880_000,currency:'CAT'});
    ...
    expect(await wallet.getBalance({userId,now})).toMatchObject({ledgerBalanceMinor:910_000,availableMinor:910_000,currency:'CAT'});

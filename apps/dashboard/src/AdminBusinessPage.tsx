...
function OrderRequirementRegion({requirements,derivedTotalMinor,currency}:{requirements:Array<Record<string,unknown>>;derivedTotalMinor:unknown;currency:string}){
  const active=requirements.filter((item)=>item.status!=='REMOVED');
  return <section className="subsection order-requirements" aria-label="订单项目需求"><div className="subsection-heading"><div><h3>项目需求</h3><p>老板提交的项目、时长与陪玩名额；实际接单人员在下方逐条对应。</p></div><strong>报价 {typeof derivedTotalMinor==='number'?formatMinorCurrency(derivedTotalMinor,currency):'—'}</strong></div>{active.length===0?<p>尚未添加项目需求。</p>:<div className="requirement-card-grid">{active.map((item)=>{const requested=Number(item.requestedPlayerCount??0);const filled=Number(item.filledPlayerCount??0);const remaining=Math.max(0,requested-filled);return <article className="requirement-detail-card" key={String(item.id)}><header><div><span>{String(item.regionDisplayName??item.region??'不限区服')}</span><h4>{String(item.gameDisplayName??item.game??'未指定游戏')} · {String(item.serviceDisplayName??item.service??'未指定服务')}</h4></div><span>{remaining===0?'名额已满':`还差 ${remaining} 位`}</span></header><dl><OrderFact label="计费" value={`${String(item.unitCount??'—')} 单位 · ${String(item.billingUnitMinutes??'—')} 分钟/单位`}/><OrderFact label="所需人数" value={`${requested} 位`}/><OrderFact label="已接单" value={`${filled} 位`}/><OrderFact label="项目小计" value={typeof item.estimatedLinePriceMinor==='number'?formatMinorCurrency(item.estimatedLinePriceMinor,currency):'—'} strong/></dl></article>;})}</div>}</section>;
}

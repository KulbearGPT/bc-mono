...
function ActionFields({ action, item, businessTagOptions, serviceCatalogOptions, dispatchCandidateOptions }: { action: AdminBusinessAction; item?:Record<string,unknown>; businessTagOptions?: BusinessTagGroups;serviceCatalogOptions?:Array<Record<string,unknown>>;dispatchCandidateOptions?:Array<Record<string,unknown>> }) {
  ...
  if(action.id==='CREATE_PACKAGE_VERSION')return <ServicePackageFields catalogs={serviceCatalogOptions??[]}/>;
...
function ServicePackageFields({catalogs}:{catalogs:Array<Record<string,unknown>>}){const[slots,setSlots]=useState<Array<{key:string;serviceCatalogVersionId:string;unitCount:number;customerNoteTemplate:string}>>([{key:crypto.randomUUID(),serviceCatalogVersionId:'',unitCount:1,customerNoteTemplate:''}]);const serialized=JSON.stringify(slots.map(({serviceCatalogVersionId,unitCount,customerNoteTemplate})=>({serviceCatalogVersionId,unitCount,customerNoteTemplate:customerNoteTemplate.trim()||null})));return <>
  <label className="field"><span>稳定代码</span><input name="code" required maxLength={100} pattern="[A-Z0-9_]{2,100}" placeholder="DELTA_ESCORT"/></label>
  <label className="field"><span>展示名称</span><input name="displayName" required maxLength={100} placeholder="三角洲护航"/></label>
  <label className="field field--full"><span>套餐说明</span><textarea name="description" required rows={3} maxLength={1000} placeholder="两只技术猫猫护航，也可以把其中一席换成聊天陪伴。"/></label>
  <label className="field"><span>套餐总价（CAT 最小单位，可选）</span><input name="defaultCustomerPriceMinor" type="number" min="1" step="1"/><small>留空时由 API 汇总席位目录价。</small></label>
  ...
  <fieldset className="field field--full package-slot-editor"><legend>默认陪玩席位（按顺序）</legend>{slots.map((slot,index)=><div className="package-slot-row" key={slot.key}><strong>{index+1} 号位</strong><label><span>服务项目</span><select required value={slot.serviceCatalogVersionId} onChange={(event)=>setSlots(current=>current.map(item=>item.key===slot.key?{...item,serviceCatalogVersionId:event.currentTarget.value}:item))}><option value="">请选择</option>{catalogs.map(catalog=><option key={String(catalog.id)} value={String(catalog.id)}>{`${String(catalog.gameDisplayName??catalog.game)} · ${String(catalog.serviceDisplayName??catalog.service)}${catalog.regionDisplayName?` · ${String(catalog.regionDisplayName)}`:''}`}</option>)}</select></label><label><span>计费单位数</span><input type="number" min="1" step="1" value={slot.unitCount} onChange={(event)=>setSlots(current=>current.map(item=>item.key===slot.key?{...item,unitCount:Number(event.currentTarget.value)}:item))}/></label><label><span>默认偏好</span><input maxLength={500} value={slot.customerNoteTemplate} placeholder="例如：负责技术护航" onChange={(event)=>setSlots(current=>current.map(item=>item.key===slot.key?{...item,customerNoteTemplate:event.currentTarget.value}:item))}/></label><button type="button" disabled={slots.length===1} onClick={()=>setSlots(current=>current.filter(item=>item.key!==slot.key))}>移除此席位</button></div>)}<button type="button" disabled={slots.length>=25} onClick={()=>setSlots(current=>[...current,{key:crypto.randomUUID(),serviceCatalogVersionId:'',unitCount:1,customerNoteTemplate:''}])}>添加陪玩席位</button><p className="field-help">每个席位都会生成一条独立需求，可分别匹配项目和陪玩。</p></fieldset>
</>}

...
function StandaloneRefundFields({item}:{item?:Record<string,unknown>}) {
  ...
  return <>
    ...
    <label className="field"><span>退款金额（CAT subunit）</span><input name="amountMinor" type="number" required min={1} max={amountMinor} step={1}/><small>当前最多可提交 {amountMinor} {currency}，最终以系统确认的可退款金额为准。</small></label>
...
function OrderCancellationResolutionFields({item}:{item?:Record<string,unknown>}) {
  ...
  return <>
    ...
    <input type="hidden" name="currency" value={currency}/>
    <label className="field"><span>退回客户（CAT subunit）</span><input name="refundAmountMinor" type="number" required min={0} max={amountMinor} step={1} defaultValue={amountMinor}/><small>最多 {amountMinor} {currency}；未扣款订单将按此金额释放预留。</small></label>
    <label className="field"><span>保留陪玩收益（CAT subunit）</span><input name="playerEarningMinor" type="number" required min={0} max={playerEarningMinor} step={1} defaultValue={0}/><small>最多 {playerEarningMinor} {currency}；依据已完成服务量人工核对。</small></label>
...
function PlayerCompensationFields({offerings}:{offerings:Array<Record<string,unknown>>}){
  ...
  return <>
    ...
    {selectedOffering&&<><div className="field field--full player-compensation-editor-heading"><span>编辑 {compensationProjectName(selectedOffering)}</span><small>本次将保存全部已缓存的项目草稿</small></div>
      ...
      {draft.type==='PERCENT_BPS'?<label className="field"><span>分成比例（%）</span><input name="percentage" type="number" required min="0.01" max="100" step="0.01" placeholder="例如 60" value={draft.percentage} onChange={(event)=>updateDraft({percentage:event.currentTarget.value})}/></label>:<label className="field"><span>每单位固定收益（CAT subunit）</span><input name="fixedAmountMinor" type="number" required min="1" step="1" value={draft.fixedAmountMinor} onChange={(event)=>updateDraft({fixedAmountMinor:event.currentTarget.value})}/></label>}
...
function GiftCatalogFields({options,item}:{options?:BusinessTagGroups;item?:Record<string,unknown>}) {
  return <>
    ...
    <label className="field"><span>价格（CAT subunit）</span><input name="amountMinor" type="number" required min={1} step={1} defaultValue={numberValue(item?.priceMinor)} /></label>
...
function ServiceCatalogFields({options,item}:{options?:BusinessTagGroups;item?:Record<string,unknown>}) {
  return <>
    ...
    <label className="field"><span>用户单价（CAT subunit）</span><input name="customerAmountMinor" type="number" required min={1} step={1} defaultValue={numberValue(item?.customerUnitPriceMinor)} /></label>

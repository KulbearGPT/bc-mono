...
function ServicePackageFields({catalogs,item}:{catalogs:Array<Record<string,unknown>>;item?:Record<string,unknown>}){const[slots,setSlots]=useState<Array<{key:string;serviceCatalogVersionId:string;unitCount:number;customerNoteTemplate:string}>>(()=>packageEditorSlots(item));const serialized=JSON.stringify(slots.map(({serviceCatalogVersionId,unitCount,customerNoteTemplate})=>({serviceCatalogVersionId,unitCount,customerNoteTemplate:customerNoteTemplate.trim()||null})));return <>
  ...
  <label className="field"><span>套餐总价（CAT 最小单位，可选）</span><input name="defaultCustomerPriceMinor" type="number" min="1" step="1" defaultValue={numberValue(item?.defaultCustomerPriceMinor)}/><small>留空时由 API 汇总席位目录价。</small></label>

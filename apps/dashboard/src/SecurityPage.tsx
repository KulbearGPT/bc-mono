...
export function SecurityPage({ capabilities }: { capabilities: DashboardCapabilities }) {
  ...
  async function beginEnrollment() {
    ...
    try {
      ...
      const body = await response.json().catch(() => null) as { data?: Enrollment; error?: { message?: string } } | null;
  ...
  async function verifyEnrollment() {
    ...
    try {
      ...
      const body = await response.json().catch(() => null) as { data?: { recoveryCodes?: string[] }; error?: { message?: string } } | null;
  ...
  async function beginStepUp() {
    ...
    try {
      ...
      const body = await response.json().catch(() => null) as { data?: Challenge; error?: { message?: string } } | null;
  ...
  async function completeStepUp(method: 'TOTP' | 'RECOVERY_CODE') {
    ...
    try {
      ...
      const body = await response.json().catch(() => null) as { data?: { validUntil: string }; error?: { message?: string } } | null;
  ...
  return (
    <section className="dashboard-page security-page" aria-labelledby="security-title">
      ...
      <div className="content-panel security-guidance"><p>超过当前层级金额上限的操作会进入审批，不会提前扣款、退款或广播。达到执行层级后，敏感操作仍需完成 15 分钟内的近期验证。</p><div className="inline-actions">
        {!capabilities.mfa?.enrolled && <button className="button-primary" type="button" disabled={busy} onClick={() => void beginEnrollment()}>绑定验证器</button>}
        <button type="button" disabled={busy} onClick={() => void beginStepUp()}>进行近期验证</button>
      </div></div>
      {(enrollment || challenge) && (
        <div className="action-panel">
          ...
          <div className="inline-actions security-proof-actions">
            {enrollment && <button className="button-primary" type="button" disabled={busy} onClick={() => void verifyEnrollment()}>确认绑定</button>}
            {challenge && <button type="button" disabled={busy} onClick={() => void completeStepUp('TOTP')}>使用验证码确认</button>}
            {challenge && <button type="button" disabled={busy} onClick={() => void completeStepUp('RECOVERY_CODE')}>使用恢复码确认</button>}
          </div>
        </div>
      )}

...
export function SandboxFundingPage(props: { capabilities: DashboardCapabilities }) {
  ...
  async function load() {
    ...
    try {
      const next = await getSandboxFundingAccount(client, userId);
  ...
  async function submit() {
    ...
    try {
      const result = await setSandboxTargetBalance(client, userId, account, Number(target));
  ...
  return (
    <section className="dashboard-page sandbox-funding-page">
      ...
      <div className="sandbox-lookup">
        <label>业务用户 ID<input value={userId} onChange={(event) => setUserId(event.target.value)} /></label>

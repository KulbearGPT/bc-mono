...
export function CustomerWalletPanel(props:{userId:string;balance:WalletBalance;entries:WalletEntry[];busy:boolean;
  nextCursor?:string|null;onNextPage?:()=>void;canTopUp:boolean;canExternalRefund:boolean;onTopUp?:(value:WalletFundingSubmission)=>void|Promise<void>;onExternalRefund?:(value:WalletFundingSubmission)=>void|Promise<void>;canAdjust?:boolean;onAdjustment?:(value:WalletAdjustmentSubmission)=>void|Promise<void>}){
  const [kind,setKind]=useState<'TOP_UP'|'CASH_REFUND_DEBIT'|'ADJUSTMENT'>(()=>props.canTopUp?'TOP_UP':props.canExternalRefund?'CASH_REFUND_DEBIT':props.canAdjust?'ADJUSTMENT':'TOP_UP');
  const canSubmit=kind==='TOP_UP'?props.canTopUp&&Boolean(props.onTopUp):kind==='CASH_REFUND_DEBIT'?props.canExternalRefund&&Boolean(props.onExternalRefund):Boolean(props.canAdjust&&props.onAdjustment);
  ...
  return <section className="wallet-panel" aria-label="客户钱包">
    ...
    <div role="group" aria-label="资金操作" className="wallet-tabs"><button type="button" aria-pressed={kind==='TOP_UP'} disabled={!props.canTopUp} title={props.canTopUp?undefined:'充值需要 L2 及以上权限'} onClick={()=>setKind('TOP_UP')}>充值</button><button type="button" aria-pressed={kind==='CASH_REFUND_DEBIT'} disabled={!props.canExternalRefund} title={props.canExternalRefund?undefined:'渠道退款需要 L2 及以上权限'} onClick={()=>setKind('CASH_REFUND_DEBIT')}>渠道退款扣款</button>{props.canAdjust&&<button type="button" aria-pressed={kind==='ADJUSTMENT'} onClick={()=>setKind('ADJUSTMENT')}>账目冲正</button>}</div>
    {!props.canTopUp&&<p className="action-unavailable">充值需要 L2 及以上权限。</p>}
    {!props.canExternalRefund&&<p className="action-unavailable">渠道退款需要 L2 及以上权限。</p>}
    {canSubmit&&<form onSubmit={submit} className="wallet-form">
      {kind==='ADJUSTMENT'?<>

...
export interface InMemoryParticipantPlayer { userId: string; displayName: string; eligible: boolean;discordUserId?:string|null;discordTag?:string|null }
...
export class InMemoryOrderParticipantStore implements OrderParticipantStore {
  ...
  private requireMutableOrder(input: ParticipantScope) { const order = this.requireScopedOrder(input); if (order.status === 'COMPLETED' || order.status === 'CANCELLED' || order.captured) throw new OrderParticipantError('BUSINESS_RULE_ERROR', 'Terminal or captured order participants are immutable.'); return order; }
}
...
export class PostgresOrderParticipantStore implements OrderParticipantStore {
  ...
  private async finishMutation(client: PoolClient, input: AddParticipantInput | UpdateParticipantInput, participant: OrderParticipantRecord, eventType: string): Promise<ParticipantMutationResult> {
    ...
    await client.query(`INSERT INTO outbox_events(id,event_type,aggregate_type,aggregate_id,order_id,dedupe_key,payload,status,row_version,attempt_count,max_attempts,available_at,created_at,updated_at) VALUES(gen_random_uuid(),'PANEL_SYNC','order',$1,$1,$2,$3::jsonb,'PENDING',1,0,8,$4,$4,$4) ON CONFLICT DO NOTHING`,[input.orderId,`${input.idempotencyKey}:panel-sync`,JSON.stringify({kind:'ORDER_PARTICIPANTS_CORRECTED_CHANNEL_SYNC',orderId:input.orderId,orderVersion:updated.rows[0].row_version}),input.now]);
  ...
  private async assertWalletIncrease(client:PoolClient,userId:string,delta:number,excludeReservationId:string|null){const wallet=await client.query<{id:string}>(`SELECT id FROM wallet_accounts WHERE user_id=$1 AND status='ACTIVE' FOR UPDATE`,[userId]);if(!wallet.rows[0])throw new OrderParticipantError('BUSINESS_RULE_ERROR','Customer wallet was not found.');const settlementJoin=reservationSettlementLateralSql('reservation','settlement');const remainingMinor=reservationRemainingMinorSql('reservation','settlement');const balance=await client.query<{available:string}>(`SELECT (COALESCE((SELECT SUM(CASE WHEN direction='CREDIT' THEN amount_minor ELSE -amount_minor END) FROM wallet_entries WHERE wallet_account_id=$1),0)-COALESCE((SELECT SUM(${remainingMinor}) FROM fund_reservations reservation ${settlementJoin} WHERE reservation.user_id=$2 AND reservation.status IN (${activeReservationStatuses.map(status=>`'${status}'`).join(',')}) AND ($3::uuid IS NULL OR reservation.id<>$3)),0))::text available`,[wallet.rows[0].id,userId,excludeReservationId]);if(safeSignedMinor(balance.rows[0]?.available??'0')<delta)throw new OrderParticipantError('BUSINESS_RULE_ERROR','Available wallet balance is insufficient for the participant price increase.');}
  ...
  private assertMutable(order: OrderScopeRow, expectedVersion: number) { if (order.row_version !== expectedVersion) throw new OrderParticipantError('CONFLICT', 'Order version is stale.'); if (order.status === 'COMPLETED' || order.status === 'CANCELLED' || order.captured) throw new OrderParticipantError('BUSINESS_RULE_ERROR', 'Terminal or captured order participants are immutable.'); }
...
interface ParticipantRow { id:string;order_id:string;order_requirement_id:string|null;player_id:string;discord_user_id:string|null;discord_tag:string|null;service_catalog_version_id:string;status:OrderParticipantStatus;row_version:number;player_display_name_snapshot:string;game_code_snapshot:string;game_display_name_snapshot:string;service_code_snapshot:string;service_display_name_snapshot:string;region_code_snapshot:string|null;region_display_name_snapshot:string|null;billing_unit_minutes_snapshot:number;unit_count:number;customer_unit_price_minor_snapshot:string|number;line_price_minor:string|number;compensation_type_snapshot:ParticipantCompensationType;compensation_value_snapshot:string|number;compensation_source:ParticipantCompensationSource;expected_earning_minor:string|number;ready_at:string|Date|null;created_at:string|Date;updated_at:string|Date }
const participantSelect=`SELECT participant.id,participant.order_id,participant.order_requirement_id,participant.player_id,participant.service_catalog_version_id,participant.status::text,participant.row_version,participant.player_display_name_snapshot,participant.game_code_snapshot,participant.game_display_name_snapshot,participant.service_code_snapshot,participant.service_display_name_snapshot,participant.region_code_snapshot,participant.region_display_name_snapshot,participant.billing_unit_minutes_snapshot,participant.unit_count,participant.customer_unit_price_minor_snapshot,participant.line_price_minor,participant.compensation_type_snapshot::text,participant.compensation_value_snapshot,participant.compensation_source::text,participant.expected_earning_minor,participant.ready_at,participant.created_at,participant.updated_at,(SELECT account.discord_user_id FROM discord_accounts account JOIN orders scoped_order ON scoped_order.id=participant.order_id AND scoped_order.guild_id=account.guild_id WHERE account.user_id=participant.player_id LIMIT 1) discord_user_id,(SELECT account.username FROM discord_accounts account JOIN orders scoped_order ON scoped_order.id=participant.order_id AND scoped_order.guild_id=account.guild_id WHERE account.user_id=participant.player_id LIMIT 1) discord_tag FROM order_participants participant`;
function mapParticipant(row:ParticipantRow):OrderParticipantRecord{return{id:row.id,orderId:row.order_id,orderRequirementId:row.order_requirement_id,playerId:row.player_id,displayName:row.player_display_name_snapshot,discordUserId:row.discord_user_id,discordTag:row.discord_tag,serviceCatalogVersionId:row.service_catalog_version_id,game:row.game_code_snapshot,gameDisplayName:row.game_display_name_snapshot,service:row.service_code_snapshot,serviceDisplayName:row.service_display_name_snapshot,region:row.region_code_snapshot,regionDisplayName:row.region_display_name_snapshot,billingUnitMinutes:Number(row.billing_unit_minutes_snapshot),unitCount:Number(row.unit_count),customerUnitPriceMinor:safeMinor(row.customer_unit_price_minor_snapshot),status:row.status,linePriceMinor:safeMinor(row.line_price_minor),compensationType:row.compensation_type_snapshot,compensationValue:safeMinor(row.compensation_value_snapshot),compensationSource:row.compensation_source,expectedEarningMinor:safeMinor(row.expected_earning_minor),readiness:row.ready_at?'READY':'NOT_READY',version:row.row_version,createdAt:new Date(row.created_at).toISOString(),updatedAt:new Date(row.updated_at).toISOString()};}

...
export interface PlayerStore {
  ...
  updateAvailability(input: { playerId: string; expectedVersion: number; availability: PlayerAvailability; now: Date }): Promise<PlayerProfileRecord> | PlayerProfileRecord;
...
export class InMemoryPlayerStore implements PlayerStore {
  ...
  findByDiscord(input: { guildId: string; discordUserId: string }): PlayerProfileRecord | null {
    return this.profiles.find((profile) => {
      return profile.guildId === input.guildId && profile.discordUserId === input.discordUserId;
    }) ?? null;
  }

  findById(playerId: string): PlayerProfileRecord | null {
    return this.profiles.find((profile) => profile.playerId === playerId) ?? null;
  }

  updateAvailability(input: { playerId: string; expectedVersion: number; availability: PlayerAvailability; now: Date }): PlayerProfileRecord {
    ...
    if (profile.reviewStatus !== 'ACTIVE') {
      throw new PlayerError('PLAYER_NOT_ELIGIBLE', 'Only ACTIVE players can change availability.');
    }
  ...
  updateOperationalStatus(input: {
    ...
    return this.replace(profile.playerId, {
      ...
      pausedAt: input.reviewStatus === 'PAUSED' ? input.now.toISOString() : profile.pausedAt,
      suspendedAt: input.reviewStatus === 'SUSPENDED' ? input.now.toISOString() : profile.suspendedAt,
      version: profile.version + 1,
      updatedAt: input.now.toISOString()
    });
  }
  ...
  private assertVersion(profile: PlayerProfileRecord, expectedVersion: number): void {
    if (profile.version !== expectedVersion) {
      throw new PlayerError('CONFLICT', 'Player profile version is stale.');
    }
  }

  private replace(playerId: string, next: PlayerProfileRecord): PlayerProfileRecord {
    const index = this.profiles.findIndex((profile) => profile.playerId === playerId);
...
export class PostgresPlayerStore implements PlayerStore {
  ...
  async findByDiscord(input: { guildId: string; discordUserId: string }): Promise<PlayerProfileRecord | null> {
    const result = await this.client.query<PlayerProfileRow>(playerProfileSelectSql({
      whereClause: `
...
WHERE profile.id = $1
  ...
  async updateAvailability(input: { playerId: string; expectedVersion: number; availability: PlayerAvailability; now: Date }): Promise<PlayerProfileRecord> {
...
WHERE id = $1
      `,
      [current.playerId, input.presence, input.observedAt, input.now.toISOString()]
    );
...
export function selectEligibleDispatchCandidates(
  ...
  return profiles.filter((profile) => {
    return profile.userStatus === 'ACTIVE'
...
export function registerPlayerRoutes(
  ...
  registerSecureWriteRoute(server, security, {
    ...
    permission: 'player.availability.manage_self',
    action: 'SET_PLAYER_AVAILABILITY',
    targetType: 'player_profile',
    acceptedSources: ['DISCORD_BOT', 'DASHBOARD'],
    handler: async (request, actor) => {
      const profile = await currentPlayer(options.store, actor);
      const body = parseAvailabilityBody(request.body);
      return toApiProfile(await options.store.updateAvailability({
  ...
  registerSecureWriteRoute(server, security, {
    ...
    handler: async (request) => {
      ...
      return {
        ...
        presence: body.presence,
        observedAt: body.observedAt,
        dispatchEligible: updated ? isGenerallyDispatchEligible(updated) : false
      };
    },
    mapError: mapPlayerError,
  ...
  registerSecureWriteRoute(server, security, {
    ...
    handler: async (request) => {
      ...
      return toApiProfile(await options.store.updateOperationalStatus({
        playerId: playerIdParam(request),
        expectedVersion: body.expectedVersion,
...
async function currentPlayer(store: PlayerStore, actor: ActorContext): Promise<PlayerProfileRecord> {
  if (!actor.guildId || !actor.discordUserId) {
    throw new PlayerError('AUTH_REQUIRED', 'Discord actor context is required.');
  }
  const profile = await store.findByDiscord({ guildId: actor.guildId, discordUserId: actor.discordUserId });
  if (!profile) {
    throw new PlayerError('PLAYER_NOT_ELIGIBLE', 'No player profile is bound to this actor.');
  }
  return profile;
}

function buildEligibility(profile: PlayerProfileRecord, now: Date, requirement: { game: string; service: string } | null) {
  const checks = [
    {
      code: 'ACTIVE_REVIEW_STATUS',
      passed: profile.reviewStatus === 'ACTIVE',
      reason: profile.reviewStatus === 'ACTIVE' ? null : `reviewStatus is ${profile.reviewStatus}`
    },
    {
      code: 'MATCHING_TAGS',
      passed: requirement ? profile.gameTags.includes(requirement.game) && profile.serviceTags.includes(requirement.service) : true,
      reason: requirement ? 'Required game and service tags must match.' : null
    },
    {
      code: 'DISCORD_ONLINE',
      passed: profile.discordPresence === 'ONLINE',
      reason: profile.discordPresence === 'ONLINE' ? null : `discordPresence is ${profile.discordPresence}`
    },
    {
      code: 'AVAILABLE',
      passed: profile.availability === 'AVAILABLE',
      reason: profile.availability === 'AVAILABLE' ? null : `availability is ${profile.availability}`
    },
    {
      code: 'NO_ACTIVE_ORDER',
      passed: !profile.activeOrderId,
      reason: profile.activeOrderId ? 'Player already has an active order.' : null
    }
  ];
  return {
...
function parseAvailabilityBody(body: unknown): { expectedVersion: number; availability: PlayerAvailability } {
  ...
  const availability = enumValue(input.availability, ['AVAILABLE', 'BUSY', 'OFFLINE'], 'availability');
...
function parsePresenceBody(body: unknown): {
  ...
  return {
    ...
    presence: enumValue(input.presence, ['ONLINE', 'IDLE', 'DND', 'OFFLINE', 'UNKNOWN'], 'presence'),
...
function parseStatusBody(body: unknown): {
  ...
  return {
    expectedVersion: positiveInteger(input.expectedVersion, 'expectedVersion'),
    reviewStatus: enumValue(input.reviewStatus, ['ACTIVE', 'PAUSED', 'SUSPENDED'], 'reviewStatus'),
    reasonCode: stringValue(input.reasonCode, 'reasonCode'),
...
function objectBody(body: unknown): Record<string, unknown> {
  if (!body || typeof body !== 'object' || Array.isArray(body)) {
    throw new PlayerError('VALIDATION_ERROR', 'Request body must be an object.');
  }
...
function positiveInteger(value: unknown, field: string): number {
  if (!Number.isInteger(value) || (value as number) < 1) {
    throw new PlayerError('VALIDATION_ERROR', `${field} must be a positive integer.`);
  }
...
function stringValue(value: unknown, field: string): string {
  if (typeof value !== 'string' || !value.trim()) {
    throw new PlayerError('VALIDATION_ERROR', `${field} must be a non-empty string.`);
  }
  return value;
}
...
function tags(value: unknown, field: string): string[] {
  if (!Array.isArray(value) || value.length === 0 || value.some((item) => typeof item !== 'string' || !item.trim())) {
    throw new PlayerError('VALIDATION_ERROR', `${field} must contain at least one tag.`);
  }
...
function mapPlayerError(error: unknown): { statusCode: number; code: string; message: string } | null {
...
function playerProfileSelectSql(input: { whereClause: string; activeStatusesParam: number }): string {
  return `

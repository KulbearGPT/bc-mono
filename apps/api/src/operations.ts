...
const l2VisibleJobTypes = new Set<JobType>(['GIFT_ANNOUNCEMENT', 'DISPATCH_MESSAGE', 'CHANNEL_ARCHIVE', 'PANEL_SYNC', 'CHANNEL_CREATE_FAILURE']);
const l3VisibleJobTypes = new Set<JobType>([...l2VisibleJobTypes, 'GIFT_EXPIRY', 'DISPATCH_TIMEOUT', 'READINESS_TIMEOUT']);
...
function assertPolicyValue(key:string,value:number,currency:string|null){
  ...
  if(amount&&currency!=='USD')throw new OperationsError('VALIDATION_ERROR','Amount policies require USD.');

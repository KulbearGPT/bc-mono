...
async function loadCommission(client: Pick<Pool,'query'> | PoolClient,id:string,forUpdate:boolean,guildId:string): Promise<CommissionRecord> {
  const result = await client.query<CommissionRow>(`SELECT c.id,c.referral_attribution_id,c.beneficiary_user_id,c.program_type_snapshot,
    ...
    WHERE c.id=$1 AND COALESCE(source_order.guild_id,to_jsonb(source_gift)->>'guild_id',gift_order.guild_id)=$2${forUpdate ? ' FOR UPDATE OF c' : ''}`, [id,guildId]);
...
function scopedCommissionIdsSql(){return`SELECT c.id FROM commissions c JOIN consumption_entries ce ON ce.id=c.source_consumption_entry_id LEFT JOIN orders source_order ON source_order.id=ce.order_id LEFT JOIN gift_requests source_gift ON source_gift.id=ce.gift_request_id LEFT JOIN orders gift_order ON gift_order.id=source_gift.order_id WHERE COALESCE(source_order.guild_id,to_jsonb(source_gift)->>'guild_id',gift_order.guild_id)=$1`;}

...
const permissionsByMinimumLevel: Record<StaffLevel, readonly string[]> = {
  ...
  L2_SUPERVISOR: ['staff_task.resolve', 'dispatch.manual', 'gift.approve', 'gift.reject', 'refund.execute', 'order.resolve', 'order.reassign', 'order.resume', 'user.read', 'player.read', 'player.tags.manage', 'catalog.read', 'gift_catalog.read', 'earnings.read', 'user.risk.manage', 'referral.read', 'job.read', 'job.retry', 'settlement.read', 'weekly_report.read', 'wallet.top_up', 'wallet.external_refund', 'customer_profile.manage', 'welcome_dm.send'],

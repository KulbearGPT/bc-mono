...
const permissionsByMinimumLevel: Record<StaffLevel, readonly string[]> = {
  L1_SUPPORT: ['staff.session.active', 'mfa.manage_self', 'step_up.execute', 'dashboard.view', 'staff_task.read', 'staff_task.claim', 'staff_task.verify', 'order.read', 'order.participants.manage', 'order.pause', 'gift.assist', 'gift_request.read', 'audit.read', 'customer_profile.read', 'customer_profile.note.append', 'wallet.read'],
  L2_SUPERVISOR: ['staff_task.resolve', 'dispatch.manual', 'gift.approve', 'gift.reject', 'refund.execute', 'order.resolve', 'order.reassign', 'order.resume', 'user.read', 'player.read', 'player.tags.manage', 'catalog.read', 'gift_catalog.read', 'earnings.read', 'user.risk.manage', 'referral.read', 'approval.read', 'approval.approve', 'approval.reject', 'job.read', 'job.retry', 'settlement.read', 'weekly_report.read', 'wallet.top_up', 'wallet.external_refund', 'customer_profile.manage', 'welcome_dm.send'],

...
export const BOT_COPY = {
  ...
  orders: {
    publicEntryIntroduction: '今晚想找一位合拍的游戏搭子？从这里提交委托即可。每位客人同一时间只能有一个进行中的订单；如果已有订单，我们会带你回到原频道。',
    ...
    dispatchStarted: '🐈 猫舍正在为你寻找合适的陪玩。我们会通知符合条件且在线可接单的成员；服务开始前取消订单，预留猫条会按规则释放。',
...
export const botCopy = {
  common: {
    retryWithRequestId: (requestId: string) => `这次操作没有完成，请稍后再试。\n如果问题持续出现，请将编号 ${requestId} 提供给猫舍前台。request_id: ${requestId}`,
    featureUnavailable: (feature: string, requestId: string) => `${feature}暂时没有打开，请稍后再试。\n如果问题持续出现，请将编号 ${requestId} 提供给猫舍前台。request_id: ${requestId}`
  },
  onboarding: {
    registrationResult: (input: { applicant: boolean; created: boolean; rolePending: boolean }) => input.applicant
  ...
  gifts: {
    ...
    shortfall: (amount: string, instructions: string) => `🐟 猫条好像不太够\n这份礼物还差 ${amount}。${instructions}完成后刷新余额，就可以继续送出。`,
    catalogTarget: (displayName: string) => `这份礼物将送给：${displayName}`,
    requestSubmitted: (giftName: string, amount: string) => `🎁 礼物已经装进猫爪包裹\n「${giftName}」已预留 ${amount}，正在等待猫舍前台核对。核对完成前不会正式扣除。`
  },
  orders: {
    reviewExpectedAt: (expiresAt: string) => `预计复核时间：${expiresAt}`,
    conflictRefreshed: (requestId: string) => `订单刚刚有了新变化，我们已经为你刷新到最新内容。request_id: ${requestId}`,
    incomplete: (requestId: string) => `委托信息还差一点，请补齐标出的内容后再确认。request_id: ${requestId}`,
    channelCreationFailed: (requestId: string, reportFailed = false) => `暂时没有成功创建订单频道，请稍后再试或联系猫舍前台。request_id: ${requestId}${reportFailed ? '\n故障记录也没有成功送达，请把这个编号提供给客服。' : ''}`,
    optionSaveFailed: (requestId: string) => `这个选项暂时没有保存成功，订单菜单已经恢复。请确认后再试。request_id: ${requestId}`,
    menuRecoveryFailed: (requestId: string) => `订单菜单暂时无法恢复，请联系猫舍前台并提供编号 ${requestId}。request_id: ${requestId}`,
  ...
  entry: {
    channelCreated: (channelMention: string) => `🐾 猫窝已经为你收拾好了\n订单频道已创建：${channelMention}\n进入频道补充需求后，就可以提交委托。`,

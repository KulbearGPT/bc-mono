...
export const botConfigBooleanFields = [
  "new_orders_enabled",
...
export type BotConfigSelectableField =
  BotConfigChannelField | BotConfigRoleField;
...
export type BotConfigManageableField =
  BotConfigSelectableField | BotConfigScalarField;
...
export class BotConfigSessionStore {
  ...
  public constructor(
    ...
    this.idFactory =
      input.idFactory ?? (() => randomUUID().replaceAll("-", "").slice(0, 12));
  ...
  public create(
    ...
    if (!actor.discordUserId)
      throw new Error("A human actor is required for a Bot config session.");
    const id = this.idFactory();
    if (!/^[A-Za-z0-9_-]{8,16}$/u.test(id))
      throw new Error("Bot config session ids must be short and URL-safe.");
    const session = {
      id,
  ...
  public require(
    ...
    if (
      ...
      throw new Error(
        "Bot config session is missing, expired, or belongs to another actor.",
      );
    }
    return session;
  }
...
export class BotConfigFlow {
  ...
  public constructor(input: {
    ...
    sessions: BotConfigSessionStore;
  }) {
    this.api = input.api;
  ...
  public chooseField(
    ...
    if (
      !snapshot?.manageableFields.includes(field) ||
      !isManageableField(field)
    ) {
      throw new Error(
        "The selected Bot config field is not manageable in this session.",
      );
    }
  ...
  public async previewValue(
    ...
    if (!session.selectedField)
      throw new Error("Select a Bot config field first.");
  ...
  public async confirm(
    ...
    if (
      ...
      throw new Error(
        "A successful Bot config preview is required before confirmation.",
      );
    }
  ...
  public async testDelivery(
    ...
    if (
      !session.selectedField ||
      !isChannelField(session.selectedField) ||
      typeof session.proposedValue !== "string"
    ) {
...
export async function reloadBotConfigCache(input: {
  ...
  for (const guildId of input.guildIds) {
    try {
      input.cache.set(
        await input.api.getBotConfig(guildId, input.actorForGuild(guildId)),
      );
      loaded += 1;
    } catch (error) {
      failed += 1;
...
export function parseBotConfigCustomId(
  customId: string,
): {
  operation:
    | "field"
  ...
  const match =
    /^bc:cfg:(field|security|value|input|modal|clear|test|confirm|cancel):([A-Za-z0-9_-]{8,16})$/u.exec(
      customId,
    );
  return match
    ? {
...
export function toDiscordBotConfigReply(
  reply: BotConfigReply,
...
function presentFieldPicker(
  ...
  return {
    content: [
      snapshot.businessEnvironment === "SANDBOX"
        ? "SANDBOX 测试环境 · 测试余额不代表真实资金"
        : null,
...
function isSelectableField(field: string): field is BotConfigSelectableField {
  return (
    ...
    (botConfigRoleFields as readonly string[]).includes(field)
  );
}
...
function isNullableRoleField(field: string) {
  return (
    field === "staff_notification_role_id" ||
    field === "operations_notification_role_id"
  );
}
function isIntegerField(
  field: string,
): field is (typeof botConfigIntegerFields)[number] {
  return (botConfigIntegerFields as readonly string[]).includes(field);
}
function isBooleanField(
  field: string,
): field is (typeof botConfigBooleanFields)[number] {
  return (botConfigBooleanFields as readonly string[]).includes(field);
}
function isTextField(
  field: string,
): field is (typeof botConfigTextFields)[number] {
...
function channelTypesForField(field: BotConfigChannelField): ChannelType[] {
  return field === "private_order_category_id" ||
...
function formatConfigValue(
  field: BotConfigManageableField,
  value: BotConfigValue | undefined,
): string {
...
function parseIntegerInput(raw: string) {
  if (!/^(?:0|[1-9][0-9]{0,5})$/u.test(raw.trim()))

...
import { orderChannelTranscriptApi, resolveTranscriptUpdateMessage } from '../../order-channel-transcript.js';
export default class MessageUpdateListener extends Listener<typeof Events.MessageUpdate> {
  ...
  public async run(_old: Message | PartialMessage, message: Message | PartialMessage) {
    try {
      ...
      await orderChannelTranscriptApi.record(resolved, 'UPDATED');
    } catch (error) {

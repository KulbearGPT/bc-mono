...
export default class BotConfigSelectHandler extends InteractionHandler {
  ...
  public override parse(interaction: Interaction) {
    if (!interaction.isStringSelectMenu() && !interaction.isChannelSelectMenu() && !interaction.isRoleSelectMenu()) return this.none();
    const route = parseBotConfigCustomId(interaction.customId);
    return route && (route.operation === 'field' || route.operation === 'security' || route.operation === 'value') ? this.some(route) : this.none();
  }

  public override async run(interaction: AnySelectMenuInteraction, route: { operation: 'field' | 'security' | 'value'; sessionId: string }): Promise<void> {
    ...
    try {
      const reply = route.operation === 'field' || route.operation === 'security'
        ? botConfigFlow.chooseField(actor, route.sessionId, interaction.values[0] ?? '')
        : await botConfigFlow.previewValue(actor, route.sessionId, parseValue(interaction.values[0] ?? ''), `discord:bot-config:validate:${interaction.id}`);
...
function parseValue(value:string){return value==='true'?true:value==='false'?false:value;}

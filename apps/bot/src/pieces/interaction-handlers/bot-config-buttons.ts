...
import { ActionRowBuilder, ModalBuilder, TextInputBuilder, TextInputStyle, type ButtonInteraction, type Interaction } from 'discord.js';
...
export default class BotConfigButtonHandler extends InteractionHandler {
  ...
  public override parse(interaction: Interaction) {
    ...
    return route && (route.operation === 'input' || route.operation === 'clear' || route.operation === 'test' || route.operation === 'confirm' || route.operation === 'cancel') ? this.some(route) : this.none();
  }

  public override async run(interaction: ButtonInteraction, route: { operation: 'input' | 'clear' | 'test' | 'confirm' | 'cancel'; sessionId: string }): Promise<void> {
    ...
    try {
      if(route.operation==='input'){
        ...
        await interaction.showModal(new ModalBuilder().setCustomId(`bc:cfg:modal:${route.sessionId}`).setTitle(input.title.slice(0,45)).addComponents(
          new ActionRowBuilder<TextInputBuilder>().addComponents(new TextInputBuilder().setCustomId('value').setLabel(input.label).setStyle(input.paragraph?TextInputStyle.Paragraph:TextInputStyle.Short).setRequired(true).setMaxLength(input.paragraph?500:6))
        ));
        return;
      }
      await interaction.deferUpdate();
      const reply = route.operation === 'confirm'
        ...
        : route.operation === 'test'
          ...
          : route.operation === 'clear'
            ? await botConfigFlow.previewValue(actor,route.sessionId,null,`discord:bot-config:validate:${interaction.id}`)
...
function actorFromInteraction(interaction: ButtonInteraction): BotConfigActorContext | null {
  return interaction.guildId ? { guildId: interaction.guildId, discordUserId: interaction.user.id, interactionId: interaction.id, clientSource: 'DISCORD_BOT' } : null;
}
